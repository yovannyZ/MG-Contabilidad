...
Public Class TIPO_CUENTA
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        ElseIf (CONTAR_REG() > 0) Then
            MessageBox.Show("El código de Tipo de Cuenta ya existe", "Ya existe", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_cod.Focus()
        Else
            Try
                ...
                CMD.ExecuteNonQuery()
                con.Close()
                MessageBox.Show("El Tipo de Cuenta se guardó", "Exito", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                LIMPIAR()
                txt_cod.Focus()
            Catch ex As Exception
                con.Close()
    ...
    Private Sub btn_modificar2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_modificar2.Click
        If (Strings.Trim(txt_cod.Text) = "") Then
            MessageBox.Show("Debe insertar el código", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_cod.Focus()
        ElseIf (Strings.Trim(txt_desc.Text) = "") Then
            MessageBox.Show("Debe insertar la descripción", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_desc.Focus()
        Else
            Try
                ...
                CMD.ExecuteNonQuery()
                con.Close()
                MessageBox.Show("El Tipo de Cuenta se actualizó", "Exito", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                TabControl1.SelectedTab = (TabPage1)
            Catch ex As Exception
                con.Close()
                MsgBox(ex.Message)
