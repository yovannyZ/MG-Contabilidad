 ...
 Private Sub btn_cancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_cancelar.Click
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If (Decimal.Parse(VERIFICACION_ELIMINAR(COD_CLASE)) > 0) Then
            MessageBox.Show("El Auxiliar tiene comprobante", "No se puede Eliminar", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            btn_nuevo.Select()
        Else
            If (Decimal.Parse((CInt(MessageBox.Show(("Eliminar: " & COD_CLASE & " " & dgw1.Item(1, dgw1.CurrentRow.Index).Value.ToString), "ESTA SEGURO DE", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
                Try
                    Dim CMD As New SqlCommand("ELIMINAR_AUXILIAR", con)
                    CMD.CommandType = CommandType.StoredProcedure
                    CMD.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = COD_CLASE
                    con.Open()
                    CMD.ExecuteNonQuery()
                    con.Close()
                Catch ex As Exception
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        ElseIf (CONTAR_REG() > 0) Then
            MessageBox.Show("El codigo de Auxiliar ya existe", "YA EXISTE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_cod.Focus()
        Else
            Try
                Dim CMD As New SqlCommand("INSERTAR_AUXILIAR", con)
                CMD.CommandType = CommandType.StoredProcedure
                CMD.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = txt_cod.Text
                CMD.Parameters.Add("@DESC_AUX", SqlDbType.VarChar).Value = txt_desc.Text
                CMD.Parameters.Add("@DESC_CORTA", SqlDbType.VarChar).Value = txt_desc2.Text
                con.Open()
                CMD.ExecuteNonQuery()
                con.Close()
                MessageBox.Show("El Auxiliar se guardó", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                LIMPIAR()
                txt_cod.Focus()
            Catch ex As Exception
                con.Close()
    ...
    Private Sub btn_modificar2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_modificar2.Click
        ...
        Else
            Try
                Dim CMD As New SqlCommand("MODIFICAR_AUXILIAR", con)
                CMD.CommandType = CommandType.StoredProcedure
                CMD.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = txt_cod.Text
                CMD.Parameters.Add("@DESC_AUX", SqlDbType.VarChar).Value = txt_desc.Text
                CMD.Parameters.Add("@DESC_CORTA", SqlDbType.VarChar).Value = txt_desc2.Text
                ...
                CMD.ExecuteNonQuery()
                con.Close()
                MessageBox.Show("El Auxiliar se actualizó", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                TabControl1.SelectedTab = TabPage1
            Catch ex As Exception
                con.Close()
                MsgBox(ex.Message)
    ...
    Function CONTAR_REG() As Integer
        ...
        Try
            Dim CMD As New SqlCommand("VERIFICAR_AUXILIAR", con)
            CMD.CommandType = CommandType.StoredProcedure
            CMD.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = txt_cod.Text
            con.Open()
    ...
    Sub DATAGRID()
        Try
            Dim cmd As New SqlCommand("MOSTRAR_AUXILIAR", con)
            Dim adap As New SqlDataAdapter(cmd)
            Dim dt As New DataTable("AUXILIAR")
            adap.Fill(dt)
            dgw1.DataSource = dt
    ...
    Function VERIFICACION_ELIMINAR(ByVal COD As Object) As String
        Dim CONT As Integer = 0
        Try
            Dim CMD As New SqlCommand("VERIFICAR_ELIMINAR_AUXILIAR", con)
            CMD.CommandType = CommandType.StoredProcedure
            CMD.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = (COD)
            con.Open()
            CONT = Integer.Parse(CMD.ExecuteScalar)
            con.Close()
        Catch ex As Exception
