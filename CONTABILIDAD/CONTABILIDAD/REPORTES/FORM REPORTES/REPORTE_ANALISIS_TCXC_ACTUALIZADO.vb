...
Public Class REPORTE_ANALISIS_TCXC_ACTUALIZADO
    Dim MES0, HOJA As String
    Dim obj As New Class1
    Dim REP2 As New REP_PENDIENTE_CXC_ACTUALIZADO
    ...
    Private Sub btn_pantalla1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_pantalla1.Click
        ...
        '    REP1.ShowDialog()
        'Else
        '    Dim VISIBLE_MAESTRO, VISIBLE_MOV As String
        '    If CH_PER.Checked = True Then VISIBLE_MAESTRO = "0" Else VISIBLE_MAESTRO = "1"
        '    If CH_PER.Checked = True Then VISIBLE_MOV = "0" Else VISIBLE_MOV = "1"
        '    REP2.CREAR_REPORTE(cbo_mes.Text, txt_desc_cta0.Text, txt_cod_cta0.Text, cbo_año.Text, MES0, "0", COD_PER, STATUS_PER, SALDOS_IMPORTE, FECHA_CONC, VISIBLE_MOV, VISIBLE_MAESTRO)
        '    REP2.ShowDialog()
        'End If
        ...
        If CH_PER.Checked = True Then VISIBLE_MOV = "0" Else VISIBLE_MOV = "1"
        'CargarDatos()
        'Llamamos al procedimiento y llenamos datos
        Dim ds As New DataSet
        Try
            Dim cmd As New SqlCommand("REPORTE_CONCILIADAS_CXC_ACTUALIZADA", con)
        ...
        While i <= filas
            ...
            CodActual = dr(8) 'dgvOrginal.Item(1, i).Value
            DocActual = dr(7)
            If i = filas Then
                drs = ds.Tables(0).Rows(i) 'DocSiguiente = dgvOrginal.Item(1, i).Value
            Else
                drs = ds.Tables(0).Rows(i + 1) 'DocSiguiente = dgvOrginal.Item(1, i + 1).Value
            End If
            CodSiguiente = drs(8)
            DocSiguiente = drs(7)
            If (DocActual = DocSiguiente And CodActual = CodSiguiente) Then
                'acumulamos
                If (dr(11) = "D") Then
                    DebeSoles += CDec(dr(12)) 'CDec(dgvOrginal.Item(3, i).Value)
                    DebeDolares += CDec(dr(13))
                Else
                    HaberSoles += CDec(dr(12)) 'Haber += CDec(dgvOrginal.Item(3, i).Value)
                ...
                If i = filas Then
                    ...
                    End If
                    'dgvFinal.Rows.Add(dgvOrginal.Item(0, i).Value, dgvOrginal.Item(1, i).Value, DebeHaber, Importe)
        ...
        End While
        '------------------------------------------
        REP2.CrearReporte(cbo_mes.Text, txt_desc_cta0.Text, txt_cod_cta0.Text, cbo_año.Text, MES0, "0", COD_PER, STATUS_PER, SALDOS_IMPORTE, FECHA_CONC, VISIBLE_MOV, VISIBLE_MAESTRO)
        REP2.ShowDialog()
    End Sub
    ...
    Private Sub ExportarWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs)
        ...
        Try
            ...
            'oHoja.Cells(5, 1) = "MONEDA" : oHoja.Cells(5, 2) = "COMPROBANTE" : oHoja.Cells(6, 2) = "FECHA"
            'oHoja.Cells(6, 3) = "AUX" : oHoja.Cells(6, 4) = "COD." : oHoja.Cells(6, 5) = "NUMERO"
            'oHoja.Cells(5, 6) = "DOCUMENTO" : oHoja.Cells(6, 6) = "FECHA" : oHoja.Cells(6, 7) = "NUMERO"
            'oHoja.Cells(6, 8) = "COD." : oHoja.Cells(5, 9) = "CTA. CTE." : oHoja.Cells(5, 10) = "NOMBRE"
            'oHoja.Cells(5, 11) = "S/.IMPORTE" : oHoja.Cells(6, 11) = "DEBE" : oHoja.Cells(6, 12) = "HABER"
            'oHoja.Cells(5, 13) = "$/IMPORTE" : oHoja.Cells(6, 13) = "DEBE" : oHoja.Cells(6, 14) = "HABER"
            'oHoja.Cells(5, 15) = "GLOSA" : oHoja.Cells(5, 16) = "T.C."

            'oHoja.Range("B5:E5").Merge() : oHoja.Range("F5:H5").Merge()
            'oHoja.Range("K5:L5").Merge() : oHoja.Range("M5:N5").Merge()

            'oHoja.Range("A:A").NumberFormat = "@"
            'oHoja.Range("B:B").NumberFormat = "dd/mm/yyyy"
            'oHoja.Range("C:E").NumberFormat = "@"
            'oHoja.Range("F:F").NumberFormat = "dd/mm/yyyy"
            'oHoja.Range("G:J").NumberFormat = "@"
            'oHoja.Range("K:N").NumberFormat = "_(* #,##0.00_);_(* (#,##0.00);_(* ""-""??_);_(@_)"
            'oHoja.Range("O:O").NumberFormat = "@"
            'oHoja.Range("P:P").NumberFormat = "_(* #,##0.0000_);_(* (#,##0.0000);_(* ""-""??_);_(@_)"
            ...
            If ch_p.Checked Then
                archivoExcel = "AnalisisCXCPendientes"
            Else
                stCon = 1
                archivoExcel = "AnalisisCXCConciliadas"
            End If
            Dim cmd As New SqlCommand("REPORTE_CONCILIADAS_CXC", con)
            cmd.CommandType = CommandType.StoredProcedure
