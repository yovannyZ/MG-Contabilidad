...
Public Class CONTROL_LETRAS_CXP
    ...
    Sub BTN_ACEPTAR_LETRAS()
        ...
        For i = 0 To cont
            If DGW_DET2.Item(6, i).Value = True Then
                Try
                    Dim command As New SqlCommand("ACEPTAR_UBICACION_NRO_BANCO_CXP2", con)
    ...
    Function VERIFICAR_MON_BCO() As Boolean
        ...
        For i = 0 To cont
            If DGW_DET.Item(6, i).Value = True Then
                If BCO0 <> DGW_DET.Item(12, i).Value Then
                    'MessageBox.Show("La moneda del Banco debe coincidir con la elegida", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Return True
                    Exit Function
                End If
    ...
    Sub BTN_UBICACION_NRO_BANCO()
        ...
        For i = 0 To cont
            If DGW_DET.Item(6, i).Value = True Then
                Try
                    Dim command As New SqlCommand("ACEPTAR_UBICACION_NRO_BANCO_CXP", con)
                    command.CommandType = CommandType.StoredProcedure
                    command.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = DGW_DET.Item(14, i).Value
                    ...
                    command.Parameters.Add("@NRO_DOC_PER", SqlDbType.VarChar).Value = DGW_DET.Item(18, i).Value
                    command.Parameters.Add("@COD_CUENTA", SqlDbType.VarChar).Value = DGW_DET.Item(16, i).Value
                    ...
                    command.Parameters.Add("@NRO_BANCO", SqlDbType.VarChar).Value = DGW_DET.Item(9, i).Value
                    command.Parameters.Add("@ST_RETENCION", SqlDbType.Char).Value = IIf(DGW_DET.Item(7, i).Value, 1, 0)
                    command.Parameters.Add("@TASA_RETENCION", SqlDbType.Decimal).Value = DGW_DET.Item(19, i).Value
                    command.Parameters.Add("@IMPORTE_RETENCION", SqlDbType.Decimal).Value = DGW_DET.Item(20, i).Value
                    con.Open()
                    command.ExecuteNonQuery()
                    con.Close()
                Catch ex As Exception
    ...
    Sub CARGAR_MOD_ACEPTACION()
        DGW_DET2.Rows.Clear()
        Try
            Dim command As New SqlCommand("MOSTRAR_LETRAS_PARA_UBI_BAN_CXP2", con)
            command.CommandType = CommandType.StoredProcedure
    ...
    Sub CARGAR_MOD_UBICACION_NRO_BANCO()
        DGW_DET.Rows.Clear()
        Try
            Dim command As New SqlCommand("MOSTRAR_LETRAS_PARA_UBI_BAN_CXP", con)
            command.CommandType = CommandType.StoredProcedure
            ...
            Dim reader As SqlDataReader = command.ExecuteReader
            Do While reader.Read
                DGW_DET.Rows.Add(reader.GetValue(0), reader.GetValue(1), reader.GetValue(2), reader.GetValue(3), _
                reader.GetValue(4), OBJ.HACER_DECIMAL(reader.GetValue(5)), reader.GetValue(6), reader.GetValue(7), _
                ...
                reader.GetValue(18), OBJ.HACER_DECIMAL(reader.GetValue(19)), OBJ.HACER_DECIMAL(reader.GetValue(20)))
            Loop
            con.Close()
        Catch ex As Exception
            con.Close()
    ...
    Sub CARGAR_AÑOS()
        ...
        End Try
    End Sub
    Private Sub btn_salir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_salir.Click, Button1.Click
        main(37) = 0
        Close()
    End Sub
    ...
    Private Sub DGW_DET_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DGW_DET.CellContentClick

    End Sub

    Private Sub DGW_DET_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DGW_DET.CurrentCellDirtyStateChanged
        If DGW_DET.IsCurrentCellDirty Then
            DGW_DET.CommitEdit(DataGridViewDataErrorContexts.Commit)
        End If
        Dim col As Int32
        Dim row As Int32
        Dim impRet As Decimal
        Dim imp As Decimal
        col = DGW_DET.CurrentCell.ColumnIndex
        row = DGW_DET.CurrentCell.RowIndex
        Select Case col
            Case 7
                If DGW_DET.Item(col, row).Value Then
                    impRet = OBJ.IMPUESTO("RET")
                    imp = DGW_DET.Item(5, row).Value
                    DGW_DET.Item(19, row).Value = impRet
                    DGW_DET.Item(20, row).Value = OBJ.HACER_DECIMAL(Math.Round(imp - (imp * (impRet / 100)), 2))
                Else
                    DGW_DET.Item(19, row).Value = OBJ.HACER_DECIMAL(0)
                    DGW_DET.Item(20, row).Value = OBJ.HACER_DECIMAL(0)
                End If
                Exit Select
        End Select
    End Sub
