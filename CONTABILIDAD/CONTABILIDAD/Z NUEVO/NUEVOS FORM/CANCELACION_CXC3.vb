...
Public Class CANCELACION_CXC3
    Dim BOTON, VAR_PRO, STATUS_PER, AGREGAR_PAGO, STATUS_P As String
    Dim COD_COMP2, COD_AUX2, STATUS_DOC, STATUS_CC, STATUS_ANA As String
    Dim ITEM_PAGO, cod_ref, COD_PROYECTO, COD_MP, COD_MONEDA As String
    Dim COD_MON_DOC, COD_AUX, COD_CC, COD_COMP, COD_CONTROL As String
    Dim COD_D_H, COD_D_H_PAGO, COD_DOC, ENLACE, DESTINO As String
    ...
    Sub CBO_AUTOMATICAS(ByVal cod As Object)
        ...
        Try
            ...
            Do While Rs3.Read
                cboAutomatica.Items.Add((Rs3.GetString(1) & "               " & Rs3.GetString(0)))
            Loop
            con.Close()
        ...
        End Try
        If (cboAutomatica.Items.Count <> 0) Then
            Label14.Text = "Automaticas"
            cboAutomatica.SelectedIndex = 0
        End If
    End Sub
    ...
    Sub GENERAR_AUTO()
        Try
            ...
            Dim j As Integer = 0
            Do While (j <= CONT)
                Dim var0 As String = (dgw_mov.Item(0, j).Value)
                Dim var1 As String = (dgw_mov.Item(1, j).Value)
                Dim var2 As String = (dgw_mov.Item(2, j).Value)
                Dim var3 As String = (dgw_mov.Item(3, j).Value)
                Dim var4 As String = (dgw_mov.Item(4, j).Value)
                Dim var5 As String = (dgw_mov.Item(5, j).Value)
                Dim var6 As String = (dgw_mov.Item(6, j).Value)
                Dim var7 As String = (dgw_mov.Item(7, j).Value)
                Dim var8 As Decimal = Decimal.Parse(dgw_mov.Item(8, j).Value)
                Dim var9 As String = (dgw_mov.Item(9, j).Value)
                Dim var10 As Decimal = Decimal.Parse(dgw_mov.Item(10, j).Value)
                Dim var11 As String = (dgw_mov.Item(11, j).Value)
                Dim var12 As String = (dgw_mov.Item(12, j).Value)
                Dim var13 As String = (dgw_mov.Item(13, j).Value)
                Dim var14 As String = (dgw_mov.Item(14, j).Value)
                Dim var15 As String = (dgw_mov.Item(15, j).Value)
                Dim var16 As String = (dgw_mov.Item(16, j).Value)
                Dim var17 As String = (dgw_mov.Item(17, j).Value)
                Dim var18 As String = (dgw_mov.Item(18, j).Value)
                Dim var19 As String = (dgw_mov.Item(19, j).Value)
                Dim var20 As String = (dgw_mov.Item(20, j).Value)
                Dim var21 As String = (dgw_mov.Item(21, j).Value)
                Dim var22 As String = (dgw_mov.Item(22, j).Value)
                Dim var23 As String = (dgw_mov.Item(23, j).Value)
                Dim var24 As String = (dgw_mov.Item(24, j).Value)
                Dim var25 As String = (dgw_mov.Item(25, j).Value)
                Dim var26 As String = (dgw_mov.Item(26, j).Value)
                Dim var27 As Decimal = Decimal.Parse(dgw_mov.Item(27, j).Value)
                Dim var28 As Decimal = Decimal.Parse(dgw_mov.Item(28, j).Value)
                Dim var29 As String = (dgw_mov.Item(29, j).Value)
                Dim var30 As String = (dgw_mov.Item(30, j).Value)
                Dim var31 As String = (dgw_mov.Item(31, j).Value)
                Dim var32 As String = (dgw_mov.Item(32, j).Value)

                If (var30.Length = 8) Then
                    ...
                    ElseIf (var7 = "H") Then
                        ...
                        cod_destino = "H"
                    End If
                    dgw_mov.Rows.Add("", var1, var2, var3, var4, var5, var6, cod_enlace, var8, var9, var10, var11, _
                    var30, "", var14, var15, var16, var17, var18, var19, var20, "", var22, var23, var24, "0", _
                    "0", var27, var28, var29, "", "", "1")

                    dgw_mov.Rows.Add("", var1, var2, var3, var4, var5, var6, cod_destino, var8, var9, var10, var11, _
                    var31, "", var14, var15, var16, var17, var18, var19, var20, "", var22, var23, var24, "0", _
                    "0", var27, var28, var29, "", "", "1")
                End If
                j += 1
            Loop
    ...
    Sub ELIMINAR_AUTO()
        ...
        Do While (I <= CONT)
            If dgw_mov.Item(22, I).Value = "1" Then
                dgw_mov.Rows.RemoveAt(I)
                I = 0
                CONT = (dgw_mov.RowCount - 1)
            Else
                I += 1
            End If
        Loop
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If (Decimal.Parse((CInt(MessageBox.Show("Desea borrar el Comprobante", " Borrar Comprobante?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
            ...
            Try
                Dim comand1 As New SqlCommand("ELIMINAR_I_CON_CXC_PAGO", con)
    ...
    Private Sub btn_grabar_pago02_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_grabar_pago02.Click
        ...
        'If txt_cod_cta_pago.Text.Trim = "" Then MessageBox.Show("Debe elegir la Cuenta del Pago", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : dtp_mp.Focus() : Exit Sub
        ...
        'If txt_nro_mp.Text.Trim = "" Then MessageBox.Show("Debe elegir el Nº de Medio de Pago", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_nro_mp.Focus() : Exit Sub
        ...
        Else
            ...
            Else
                ...
                ElseIf (COD_MON_DOC = "D") Then
                    imp_pago = Math.Round((IMP_S * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                Else
                    'SOLES O AJUSTE
                ...
                ENLACE = ""
                DESTINO = ""
                DESTINO = Strings.Mid((cboAutomatica.SelectedItem), 1, 8)
                ENLACE = Strings.Mid((cboAutomatica.SelectedItem), 24, 31)
                ...
                Else
                    ...
                    txt_desc_per.Text, txt_nro_doc_per.Text, imp_pago, IMP_S, COD_MON_DOC, CBO_D_H.Text, txt_cambio_pago.Text, _
                    dtp_mp.Value, txt_glosa.Text, txt_cod_cta.Text, dtp_vcto.Value, COD_CC, COD_CONTROL, COD_PROYECTO, STATUS_CC, _
                    "1", STATUS_P, STATUS_PER, VAR_PRO, cod_ref, txt_nro_ref.Text, ENLACE, DESTINO, "0")
                    HALLAR_TOTAL()
                    LIMPIAR_DETALLES()
                    txt_cod1.Focus()
                End If
    ...
    Private Sub btn_nuevo_doc_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_nuevo_doc.Click
        ...
        If STATUS_ANA = "P" AndAlso ch_a.Checked Then
            MessageBox.Show("La Cuenta es por Pagar", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            ch_a.Focus()
            Exit Sub
        End If
        ...
        If STATUS_ANA = "P" OrElse STATUS_ANA = "C" Then
            If txt_cod_per_cab.Text.Trim = "" Then MessageBox.Show("Debe ingresar el Proveedor ", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_cod_per_cab.Focus() : Exit Sub
        End If

        If dtp_vcto.Enabled = True Then
            If dtp_mp.Value.Date > dtp_vcto.Value.Date Then
                MessageBox.Show("La fecha de Vcto. debe ser mayor o igual al Documento", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Information)
                dtp_vcto.Focus()
                Exit Sub
            End If
        End If
    ...
    Private Sub CANCELACION_CXC3_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        If TabControl1.SelectedIndex <> 0 Then
            ValidarFicha = True
        End If
    End Sub

    Private Sub CANCELACION_CXC3_Deactivate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Deactivate
        If TabControl1.SelectedIndex <> 0 Then
            ValidarFicha = False
        End If
    End Sub
    ...
    Sub HALLANDO_TOTALES()
        Dim I, CONT As Integer
        Dim TS, TD As Decimal
        TS = 0 : TD = 0
        CONT = dgw_pago0.RowCount - 1
        For I = 0 To CONT
            If dgw_pago0.Item(7, I).Value = "D" Then
                TD = TD + dgw_pago0.Item(6, I).Value
            ElseIf dgw_pago0.Item(7, I).Value = "S" Then
                TS = TS + dgw_pago0.Item(6, I).Value
            End If
        Next
        txt_soles.Text = obj.HACER_DECIMAL(Math.Round(TS, 3, MidpointRounding.AwayFromZero))
        txt_dolares.Text = obj.HACER_DECIMAL(Math.Round(TD, 3, MidpointRounding.AwayFromZero))
    End Sub
    ...
    Sub CARGAR_COMPROBANTE()
        ...
        dgw_mov.Rows.Clear()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_T_CON_CXC_PAGO", con)
            ...
            Do While rs3.Read
                If rs3.GetValue(26) = "1" Then
                    dgw_pago0.Rows.Add(rs3.GetValue(0), rs3.GetValue(28), rs3.GetValue(12), rs3.GetValue(27), _
                    rs3.GetValue(2), rs3.GetValue(3), rs3.GetValue(8), rs3.GetValue(9), rs3.GetValue(10), rs3.GetValue(25), _
                    rs3.GetValue(22), (rs3.GetValue(23)), (rs3.GetValue(24)), rs3.GetValue(11), rs3.GetValue(1), _
                    rs3.GetValue(2), rs3.GetValue(4), rs3.GetValue(5), rs3.GetValue(6), rs3.GetValue(33), rs3.GetValue(17))
                End If
                ...
                rs3.GetValue(23), rs3.GetValue(24), rs3.GetValue(25), rs3.GetValue(26), rs3.GetValue(31), rs3.GetValue(32), _
                rs3.GetValue(33), rs3.GetValue(34), rs3.GetValue(35), rs3.GetValue(36))
            Loop
            con.Close()
        Catch ex As Exception
            con.Close()
    ...
    Sub CARGAR_DETALLES(ByVal FILA0 As Object)
        ...
        txt_imp_soles.Text = txt_imp_soles.Text
        ...
        Try
            dtp_vcto.Value = dgw_mov1.Item(17, Integer.Parse(FILA0)).Value.ToString
        Catch ex As Exception
    ...
    Sub CARGAR_PAGO(ByVal FILA0 As Object)
        ...
        If dgw_pago0.Item(19, Integer.Parse(FILA0)).Value.ToString = "1" Then
            ch_a.Checked = True
        ...
        End If
        Try
            dtp_vcto.Value = dgw_pago0.Item(20, Integer.Parse(FILA0)).Value.ToString
        Catch ex As Exception
        End Try
            ...
            If dgw_mov.Item(25, k1).Value = ITEM_PAGO And dgw_mov.Item(26, k1).Value.ToString = "0" Then
                ...
                ElseIf (COD_MONEDA = "D") Then
                    If dgw_mov.Item(9, k1).Value.ToString = "A" Then
                        PAGO = 0
                    Else
                        PAGO = Math.Round((dgw_mov.Item(8, k1).Value / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                    End If
                Else
                    If dgw_mov.Item(9, k1).Value.ToString = "A" Then
                        PAGO = dgw_mov.Item(8, k1).Value
                    Else
                        PAGO = Math.Round((dgw_mov.Item(8, k1).Value * txt_cambio_pago.Text), 2, MidpointRounding.ToEven)
                    End If
                End If
                With dgw_mov
                    dgw_mov1.Rows.Add((dgw_mov1.RowCount + 1), .Item(1, k1).Value, .Item(2, k1).Value, _
                    .Item(4, k1).Value, .Item(5, k1).Value, .Item(6, k1).Value, PAGO, .Item(8, k1).Value, _
                    .Item(9, k1).Value, .Item(7, k1).Value, .Item(10, k1).Value, .Item(3, k1).Value, _
                    .Item(11, k1).Value, .Item(12, k1).Value, .Item(17, k1).Value, .Item(13, k1).Value, _
                    .Item(14, k1).Value, .Item(15, k1).Value, .Item(18, k1).Value, .Item(19, k1).Value, _
                    .Item(20, k1).Value, .Item(16, k1).Value, .Item(21, k1).Value, .Item(22, k1).Value, _
                    .Item(23, k1).Value, .Item(30, k1).Value, .Item(31, k1).Value, .Item(32, k1).Value)
                End With
            End If
            k1 += 1
        Loop
    ...
    Private Sub dgw_cta_pago_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles dgw_cta_pago.KeyDown
        ...
        If (e.KeyData = Keys.Return) Then
            ...
            '----------------------------------------------
            If STATUS_ANA = "C" OrElse STATUS_ANA = "P" Then
    ...
    Private Sub DGW_CTA1_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles DGW_CTA1.KeyDown
        If (e.KeyData = Keys.Return) Then
            ...
            CBO_AUTOMATICAS(txt_cod_cta.Text)
            If (STATUS_CC = "1") Then
                CBO_CC.Enabled = True
            End If
            If (STATUS_P = "1") Then
    ...
    Sub HALLAR_TOTAL()
        ...
        Do While (I <= CONT)
            With dgw_mov1
                If (.Item(9, I).Value.ToString = "D") Then
                    Select Case .Item(8, I).Value.ToString
                        Case "D"
                            D_DOLARES = D_DOLARES + Math.Round(CDec(.Item(7, I).Value), 2, MidpointRounding.AwayFromZero)
                            D_SOLES = D_SOLES + Math.Round((CDec(.Item(7, I).Value) * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                        Case "S"
                            D_DOLARES = D_DOLARES + Math.Round((CDec(.Item(7, I).Value) / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                            D_SOLES = D_SOLES + Math.Round(CDec(.Item(7, I).Value), 2, MidpointRounding.AwayFromZero)
                        Case "A"
                ...
                Else
                    Select Case dgw_mov1.Item(8, I).Value.ToString
                        Case "D"
                            H_DOLARES = H_DOLARES + Math.Round(CDec(.Item(7, I).Value), 2, MidpointRounding.AwayFromZero)
                            H_SOLES = H_SOLES + Math.Round((CDec(.Item(7, I).Value) * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                        Case "S"
                            H_DOLARES = H_DOLARES + Math.Round((CDec(.Item(7, I).Value) / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                            H_SOLES = H_SOLES + Math.Round(CDec(.Item(7, I).Value), 2, MidpointRounding.AwayFromZero)
                        Case "A"
        ...
        Do While (K <= CONT)
            If OFR.DGW_CAB.Item(9, K).Value.ToString = "True" Then
                ...
                If (OFR.DGW_CAB.Item(6, K).Value.ToString = COD_MONEDA) Then
                    imp_pago = (OFR.DGW_CAB.Item(8, K).Value)
                ElseIf ((COD_MONEDA = "S") Or (COD_MONEDA.ToString = "System.Data.DataRowView")) Then
                    imp_pago = obj.HACER_DECIMAL(Math.Round((OFR.DGW_CAB.Item(8, K).Value * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero))
                Else
                    imp_pago = obj.HACER_DECIMAL(Math.Round((OFR.DGW_CAB.Item(8, K).Value / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero))
                End If

                With OFR.DGW_CAB
                    CBO_AUTOMATICAS(.Item(14, K).Value)
                    ENLACE = ""
                    DESTINO = ""
                    DESTINO = Strings.Mid((cboAutomatica.SelectedItem), 1, 8)
                    ENLACE = Strings.Mid((cboAutomatica.SelectedItem), 24, 31)
                    dgw_mov1.Rows.Add((dgw_mov1.RowCount + 1).ToString("0000"), .Item(0, K).Value, _
                    .Item(2, K).Value, .Item(3, K).Value, .Item(4, K).Value, .Item(11, K).Value, _
                    imp_pago, .Item(8, K).Value, .Item(6, K).Value, .Item(10, K).Value, txt_cambio_pago.Text, _
                    .Item(5, K).Value, .Item(4, K).Value, .Item(14, K).Value, .Item(5, K).Value, COD_CC, _
                    COD_CONTROL, COD_PROYECTO, .Item(15, K).Value, .Item(16, K).Value, .Item(17, K).Value, _
                    "0", "S", "", "", ENLACE, DESTINO, "0")
                End With
    ...
    Sub INSERTAR_DGW_PAGO()
        ...
        cod_ref = obj.COD_DOC(cbo_tipo_ref.Text)
        If ch_a.Checked = True Then
            AGREGAR_PAGO = "1"
        Else
            AGREGAR_PAGO = "0"
        End If
        dgw_pago0.Rows.Add(COD_MP, cbo_mp.Text, txt_cod_cta_pago.Text, txt_desc_cta_pago.Text, txt_nro_mp.Text, dtp_mp.Value, TXT_PAGO.Text, COD_MONEDA, txt_cambio_pago.Text, ITEM0, COD_DOC_EQ, txt_nro_mp.Text, dtp_mp.Value, txt_glosa_pago.Text, COD_DOC_EQ, txt_nro_mp.Text, txt_cod_per_cab.Text, txt_des_per_cab.Text, txt_doc_per_cab.Text, AGREGAR_PAGO, dtp_vcto.Value)
        ...
        Do While (I <= CONT)
            ...
            Select Case dgw_mov1.Item(8, I).Value.ToString
                Case "D"
                    impdolares = Math.Round(Double.Parse(dgw_mov1.Item(7, I).Value), 2)
                    impsoles = Math.Round(Double.Parse(dgw_mov1.Item(7, I).Value * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                Case "S"
                    impdolares = Math.Round(Double.Parse(dgw_mov1.Item(7, I).Value / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)
                    impsoles = Math.Round(Double.Parse(dgw_mov1.Item(7, I).Value), 2)
                Case "A"
                    impdolares = 0
                    impsoles = Math.Round(Double.Parse(dgw_mov1.Item(7, I).Value), 2)
            End Select
            ...
            With dgw_mov1
                dgw_mov.Rows.Add(COD_MP, cod_doc22, .Item(2, I).Value, dtp_mp.Value, .Item(3, I).Value, .Item(4, I).Value, _
                .Item(5, I).Value, .Item(9, I).Value, .Item(7, I).Value, .Item(8, I).Value, txt_cambio_pago.Text, _
                .Item(12, I).Value, .Item(13, I).Value, .Item(15, I).Value, .Item(16, I).Value, .Item(17, I).Value, _
                CTE, .Item(14, I).Value, .Item(18, I).Value, .Item(19, I).Value, .Item(20, I).Value, .Item(22, I).Value, _
                dgw_mov1.Item(23, I).Value, .Item(24, I).Value, dtp_mp.Value, ITEM0, "0", impsoles, impdolares, "0", _
                .Item(25, I).Value, .Item(26, I).Value, "0")
            End With
            I += 1
        Loop
        '-------------------llena cabecera
        dgw_mov.Rows.Add(COD_MP, COD_DOC_EQ, txt_nro_mp.Text, dtp_mp.Value, txt_cod_per_cab.Text, _
        txt_des_per_cab.Text, txt_doc_per_cab.Text, COD_D_H_PAGO, TXT_PAGO.Text, COD_MONEDA, _
        txt_cambio_pago.Text, txt_glosa_pago.Text, txt_cod_cta_pago.Text, " ", " ", " ", "0", _
        dtp_vcto.Value, "", "", "", "", cod_ref, txt_nro_ref.Text, dtp_mp.Value, ITEM0, "1", _
        txt_saldo_soles.Text, txt_saldo_dolares.Text, AGREGAR_PAGO, "", "", "0")
    ...
    Function INSERTAR_TODO() As String
        ...
        Do While (I <= CONT)
            ...
            With dgw_mov
                ...
                .Item(3, I).Value, .Item(22, I).Value, .Item(23, I).Value, .Item(24, I).Value, _
                .Item(11, I).Value, .Item(12, I).Value, SOLES, DOLARES, .Item(7, I).Value, _
                .Item(9, I).Value, .Item(10, I).Value, .Item(13, I).Value, .Item(14, I).Value, _
                .Item(15, I).Value, " ", .Item(17, I).Value, .Item(29, I).Value, .Item(30, I).Value, _
                .Item(31, I).Value, .Item(32, I).Value, VAR_PRO00, "0", .Item(26, I).Value, _
                .Item(0, I).Value, .Item(23, I).Value, .Item(3, I).Value, .Item(25, I).Value, "", _
                VAR_PRO00, dtp_com.Value, 0, NOMBRE_PC)
            End With
            'DT_DET.Rows.Add(AÑO, MES, COD_AUX, COD_COMP, txt_nro_comp.Text, dgw_mov.Item(1, I).Value, dgw_mov.Item(2, I).Value, (dgw_mov.Item(4, I).Value), (dgw_mov.Item(6, I).Value), (I + 1).ToString("0000"), (dgw_mov.Item(3, I).Value), (dgw_mov.Item(22, I).Value), (dgw_mov.Item(23, I).Value), (dgw_mov.Item(24, I).Value), (dgw_mov.Item(11, I).Value), (dgw_mov.Item(12, I).Value), SOLES, DOLARES, (dgw_mov.Item(7, I).Value), (dgw_mov.Item(9, I).Value), (dgw_mov.Item(10, I).Value), (dgw_mov.Item(13, I).Value), (dgw_mov.Item(14, I).Value), (dgw_mov.Item(15, I).Value), " ", (dgw_mov.Item(17, I).Value), dgw_mov.Item(29, I).Value, "", "", "0", VAR_PRO00, "0", dgw_mov.Item(26, I).Value.ToString, (dgw_mov.Item(0, I).Value), dgw_mov.Item(23, I).Value.ToString, (dgw_mov.Item(3, I).Value), (dgw_mov.Item(25, I).Value), "", VAR_PRO00, dtp_com.Value, 0, NOMBRE_PC)
        ...
        End Try
        Try
            Dim comand1 As New SqlCommand("INSERTAR_I_CON_CXC_PAGO", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
    ...
    Function MODIFICAR_TODO() As String
        ...
        Do While (I <= CONT)
            ...
            SOLES = Math.Round(Decimal.Parse((dgw_mov.Item(27, I).Value)), 2)
            DOLARES = Math.Round(Decimal.Parse((dgw_mov.Item(28, I).Value)), 2)
            If (dgw_mov.Item(26, I).Value = "1") Then
                VAR_PRO00 = "0"
            Else
                VAR_PRO00 = dgw_mov.Item(21, I).Value
            End If
            Dim er, ar As String
            er = dgw_mov.Item(29, I).Value
            ar = dgw_mov.Item(26, I).Value
        ...
        End Try
        Try
            Dim comand1 As New SqlCommand("MODIFICAR_I_CON_CXC_PAGO", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
            ...
            Else
                ...
                Do While (i <= CONT)
                    If (txt_cod_cta_pago.Text.ToLower = dgw_cta_pago.Item(0, i).Value.ToString.ToLower) Then
                        ...
                        'If (STATUS_ANA = "P" OrElse STATUS_ANA = "C") AndAlso txt_cod_cta_pago.Text.Length = 8 Then
                        '    MessageBox.Show("La Cuenta es por Pagar", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        '    txt_cod_cta_pago.Clear()
                        '    txt_desc_cta_pago.Clear()
                        '    txt_cod_cta_pago.Focus()
    ...
    Private Sub btnImportarExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnImportarExcel.Click
        ...
        Dim ConExcel As String = String.Empty
        Dim ds As New DataSet
        Using ofd As New OpenFileDialog
            ofd.Filter = "Archivos*.xlsx|*.xlsx"
            If ofd.ShowDialog = Windows.Forms.DialogResult.OK Then
                RutaArchivo = ofd.FileName
                'Asiganmos la cadena de conexión
                ConExcel = "Provider=Microsoft.ACE.Oledb.12.0;Extended Properties='TEXT;HDR=Yes';" & _
                           "Data Source=" & RutaArchivo & ";Extended Properties= Excel 8.0"
                Try
                    Using con As New OleDbConnection(ConExcel)
                        Dim cmd As New OleDbCommand("Select * from [Cancelaciones$]", con)
        ...
        If ds.Tables.Count > 0 AndAlso ds.Tables(0).Rows.Count > 0 Then
            '-------------------Creación del formulario a mostrar los registros que no pasaron----
            Dim frm As New Form
            frm.Text = "Lista de registros no admitidos"
            Dim dgv As New DataGridView
            dgv.AllowUserToAddRows = False
            Dim col1, col2, col3, col4 As New DataGridViewTextBoxColumn
            col1.HeaderText = "DOC PER"
            col2.HeaderText = "NRO_DOC"
            col3.HeaderText = "TIPO DE ERROR"
            col4.HeaderText = "FILA"
            ...
            For Each dr In ds.Tables(0).Rows
                'Validamos el código documento
                If (dr(1).ToString = "") Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Falta el código de documento", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Validamos el N° Documento
                If (dr(2).ToString = "") Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Falta el número de documento", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Validamos la fecha del documento debe ser del mismo día
                If (CDate(dr(3).ToString) <> dtp_mp.Value) Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Fecha del documento inválida", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Validamos el importe
                If (dr(4).ToString = "" OrElse CDbl(dr(4)) <= 0) Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Falta el monto o este es menor o igual a 0", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Validamos el código de moneda
                If (dr(5).ToString = "" AndAlso dr(5).ToString <> "S" And dr(5).ToString <> "D" And dr(5).ToString <> "A") Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Código de moneda no valido", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Pasamos el nro doc persona al text box y hacemos todo el proceso de inserción
                Dim CONT As Integer = (DGW_PER.RowCount - 1)
                Dim i As Integer = 0
                txt_nro_doc_per.Text = dr(0).ToString.Trim
                If (dr(0).ToString.Trim = "") Then
                    txt_cod1.Text = "00000"
                    txt_cod1_LostFocus1(sender, Nothing)
                Else
                    Do While (i <= CONT)
                        If (txt_nro_doc_per.Text = Strings.Mid((DGW_PER.Item(2, i).Value), 1, Strings.Len(txt_nro_doc_per.Text)).ToLower) Then
                            'DGW_PER.CurrentCell = DGW_PER.Rows.Item(i).Cells.Item(0)
                            txt_cod1.Text = DGW_PER.Item(0, i).Value.ToString
                            txt_desc_per.Text = DGW_PER.Item(1, i).Value.ToString
                            txt_nro_doc_per.Text = DGW_PER.Item(2, i).Value.ToString
                ...
                'Validamos si llegamos al mismo número del contador
                If (txt_cod1.Text.Trim = "") Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "El número de documento de la persona no existe", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Llenamos la grilla para los datos de los doc pdtes de esa persona
                '---------------------------------------------
                DGW_CAB.Rows.Clear()
                Try
                    Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_COBRAR_PTES", con)
                    PROG_01.CommandType = (CommandType.StoredProcedure)
                    PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = obj.HALLAR_SUCURSAL(COD_AUX, COD_COMP)
                    ...
                    Dim Rs3 As SqlDataReader = PROG_01.ExecuteReader
                    Do While Rs3.Read
                        DGW_CAB.Rows.Add((Rs3.GetValue(0)), Rs3.GetValue(1), Rs3.GetValue(2), Rs3.GetValue(3), Rs3.GetValue(4), Rs3.GetValue(5), Rs3.GetValue(6), Rs3.GetValue(7), Rs3.GetValue(8), Rs3.GetValue(9), Rs3.GetValue(10), (Rs3.GetValue(11)), (Rs3.GetValue(12)), (Rs3.GetValue(13)), (Rs3.GetValue(14)), (Rs3.GetValue(15)), (Rs3.GetValue(16)), (Rs3.GetValue(17)))
                    Loop
                    con.Close()
                Catch ex As Exception
                    con.Close()
                ...
                End Try
                '-------------------------------------------
                'Validamos que el documento no se repita
                If VALIDAR_DET(dr(1).ToString, dr(2).ToString, txt_cod1.Text) = False Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Documento duplicado", indice + 1)
                    indice += 1
                    Continue For
                End If
                'Buscamos el nro de documento en la grilla
                Dim Filas As Integer = (DGW_CAB.Rows.Count - 1)
                Dim K As Integer = 0
                Dim imp_pago As Decimal = 0
                Do While (K <= Filas)
                    'Buscamos el registro que cumpla con las condiciones OFR.DGW_CAB.Item(9, K).Value.ToString = "True"
                    If DGW_CAB.Item(0, K).Value.ToString = dr(1).ToString AndAlso _
                       DGW_CAB.Item(2, K).Value.ToString = dr(2).ToString AndAlso _
                       DGW_CAB.Item(8, K).Value >= CDbl(dr(4)) AndAlso _
                       DGW_CAB.Item(6, K).Value.ToString = dr(5).ToString Then

                        If (DGW_CAB.Item(6, K).Value.ToString = COD_MONEDA) Then
                            imp_pago = (dr(4)) 'OFR.DGW_CAB.Item(8, K)
                        ElseIf ((COD_MONEDA = "S") Or (COD_MONEDA.ToString = "System.Data.DataRowView")) Then
                            imp_pago = obj.HACER_DECIMAL(Math.Round((dr(4) * txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)) 'DGW_CAB.Item(8, K)
                        Else
                            imp_pago = obj.HACER_DECIMAL(Math.Round((dr(4) / txt_cambio_pago.Text), 2, MidpointRounding.AwayFromZero)) 'DGW_CAB.Item(8, K)
                        End If
                        COD_CC = "" : COD_CONTROL = "" : COD_PROYECTO = ""
                        With DGW_CAB
                            CBO_AUTOMATICAS(.Item(14, K).Value)
                            ENLACE = ""
                            DESTINO = ""
                            ...
                            dgw_mov1.Rows.Add((dgw_mov1.RowCount + 1).ToString("0000"), .Item(0, K).Value, _
                            .Item(2, K).Value, .Item(3, K).Value, .Item(4, K).Value, .Item(11, K).Value, _
                            imp_pago, dr(4), .Item(6, K).Value, .Item(10, K).Value, txt_cambio_pago.Text, _
                            .Item(5, K).Value, .Item(4, K).Value, .Item(14, K).Value, .Item(5, K).Value, COD_CC, _
                            COD_CONTROL, COD_PROYECTO, .Item(15, K).Value, .Item(16, K).Value, .Item(17, K).Value, _
                            "0", "S", "", "", ENLACE, DESTINO, "0")
                        End With
                ...
                Loop
                If (imp_pago = 0) Then
                    dgv.Rows.Add(dr(0).ToString, dr(2).ToString, "Monto mayor al importe del documento o documento inexistente", indice + 1)
                    indice += 1
                    Continue For
                End If
                HALLAR_TOTAL()
