...
Public Class RETENCION_CXP
    ...
    Dim OFR As New CONSULTA_PCTAS_PAGAR_RET
    Dim OFR_RET As New REFERENCIA_RETENCION
    Dim reporte As New CrystalReport1
    Dim REP As New REP_RET
    Private ValidarFicha As Boolean = True
    Dim DIFERENCIA As Decimal
    Dim RetencionElectronica As Boolean = False
    Dim StatusRetencionElectronica As String = String.Empty
    Public ValorSoles As Decimal = 0
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If (Decimal.Parse((CInt(MessageBox.Show("Desea borrar el Comprobante", " Borrar Comprobante?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
            ...
            Try
                Dim comand1 As New SqlCommand("ELIMINAR_I_CON_CXP_RETENCION", con)
    ...
    Private Sub btn_grabar_pago01_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_grabar.Click
        ...
        '---------------------------------------------------------
        If VERIFICAR_CUADRE_DH() = False Then MessageBox.Show(String.Format("El comprobante no cuadra por {0}", DIFERENCIA)) : Exit Sub
        ...
        If (INSERTAR_TODO() = "EXITO") Then
            ...
            If chkStatusRetencionElectronica.Checked Then
                CrearPdf()
            End If
        Else
            obj.ELIMINAR_TEMPORAL("TCON")
            obj.ELIMINAR_TEMPORAL("TRET")
            MessageBox.Show("Ocurrio un error", "Vuelva a Intentarlo", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Return
        End If
        '---------------------------------------------------------
    ...
    Function VERIFICAR_CUADRE_DH() As Boolean
        Dim RSLT As Boolean = False
        '---------------------------------
        '---------------------------------
        Dim TOTAL_SOLES_DEBE, TOTAL_SOLES_HABER As Decimal
        Dim I As Integer = 0
        ...
        Do While (I <= CONT)
            ...
            If dgw_mov.Item(7, I).Value = "D" Then
                TOTAL_SOLES_DEBE += SOLES
            Else
                TOTAL_SOLES_HABER += SOLES
            End If
            I += 1
        Loop
        DIFERENCIA = Format(TOTAL_SOLES_DEBE - TOTAL_SOLES_HABER, "###,##0.00")
        If TOTAL_SOLES_DEBE <> TOTAL_SOLES_HABER Then RSLT = False Else RSLT = True
        '------------------------------------
        '------------------------------------
        Return RSLT
    End Function
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        Else
            ...
            'imp_pago = obj.HACER_DECIMAL(IMP_S / txt_cambio_pago.Text)
        End If
        imp_pago = Math.Round(imp_pago, 2, MidpointRounding.AwayFromZero)
        COD_CONTROL = ""
        COD_PROYECTO = ""
        COD_CC = ""
        ...
        Else
            dgw_mov1.Rows.Add((dgw_mov1.RowCount + 1).ToString("0000"), COD_DOC, txt_nro_doc.Text, imp_pago, IMP_S, COD_MON_DOC, _
    ...
    Private Sub CANCELACION_CXP_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        If obj.DIR_TABLA_DESC("R_ELEC", "TDEFA") = "SI" Then
            RetencionElectronica = True
            chkStatusRetencionElectronica.Checked = True
        End If
        KeyPreview = True
        DATAGRID()
        CBO_MP00()
    ...
    Sub CARGAR_COMPROBANTE()
        ...
        dgw_mov.Rows.Clear()
        DGW_RET2.Rows.Clear()
        If dgw1.Item(6, dgw1.CurrentRow.Index).Value.ToString = "1" Then
            chkStatusRetencionElectronica.Checked = True
        Else
            chkStatusRetencionElectronica.Checked = False
        End If
        'DataGridView1.Rows.Clear()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_T_CON_CXP_CANCELACION", con)
            ...
            Do While rs3.Read
                dgw_mov.Rows.Add(rs3.GetValue(0), rs3.GetValue(1), rs3.GetValue(2), rs3.GetValue(3), rs3.GetValue(4), _
                    ...
                    rs3.GetValue(23), rs3.GetValue(24), rs3.GetValue(25), rs3.GetValue(26), _
                    rs3.GetValue(31))
            Loop
            con.Close()
        Catch ex As Exception
        ...
        ' I RETENCIONES
        Try
            Dim pro_02 As New SqlCommand("CARGAR_I_RETENCIONES", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            pro_02.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            pro_02.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
        ...
        ' T RETENCIONES
        Try
            Dim pro_02 As New SqlCommand("CARGAR_T_RETENCIONES", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            pro_02.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            pro_02.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
        ...
        End Try
        '------------------
        cbo_mp.Text = obj.DESC_MP(COD_MP)
        '------------------
    End Sub
        ...
        For I = 0 To CONT
            If dgw_mov.Item(26, I).Value.ToString = "0" Then
                ...
                ElseIf dgw_mov.Item(9, I).Value.ToString = "A" Then
                    PAGO = Decimal.Parse(dgw_mov.Item(8, I).Value)
                Else
                    PAGO = dgw_mov.Item(27, I).Value 'Math.Round(Decimal.Parse(Decimal.Multiply(dgw_mov.Item(8, I).Value, txt_cambio_pago.Text)), 2)
                End If
                With dgw_mov
    ...
    Sub CREAR_DATASET()
        ...
        DT_RET.Columns.Add("NRO_PAGO")
    End Sub

    Sub DATAGRID()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ICON_CXP_RET", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@fe_año", SqlDbType.VarChar).Value = AÑO
            pro_02.Parameters.Add("@fe_mes", SqlDbType.VarChar).Value = MES
            pro_02.Parameters.Add("@TIPO", SqlDbType.VarChar).Value = "CPRET"
        ...
        '---------------------------anuladas
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ICON_CXP_RET_ANUL2", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@fe_año", SqlDbType.VarChar).Value = AÑO
            pro_02.Parameters.Add("@fe_mes", SqlDbType.VarChar).Value = MES
            pro_02.Parameters.Add("@TIPO", SqlDbType.VarChar).Value = "CPRET"
    ...
    Sub INSERTAR_DE_DAILOG()
        ...
        cbo_moneda1.Text = obj.DESC_MONEDA(COD_MON_DOC)
        '---------------------------------------------
        Dim RET As Decimal = obj.IMPUESTO("RET")
        txt_imp_soles.Text = Math.Round(CDec(((OFR.DGW_CAB.Item(17, ITEM0).Value * RET) / 100)), 2, MidpointRounding.AwayFromZero)
        txt_imp_doc.Text = OFR.DGW_CAB.Item(17, ITEM0).Value
        '---------------------------------------------
        txt_cod_cta.Text = OFR.DGW_CAB.Item(13, ITEM0).Value
        txt_desc_cta.Text = obj.DESC_CUENTA(txt_cod_cta.Text, AÑO)
    ...
    Function INSERTAR_TODO() As String
        ...
        Do While (I <= CONT)
            ...
            If dgw_mov.Item(9, I).Value = "S" Then
                SOLES = Math.Round(Decimal.Parse((dgw_mov.Item(8, I).Value)), 2)
                DOLARES = txt_saldo_dolares.Text 'Math.Round(Decimal.Divide(Decimal.Parse((dgw_mov.Item(8, I).Value)), Decimal.Parse((dgw_mov.Item(10, I).Value))), 2)
            Else
                DOLARES = Math.Round(Decimal.Parse((dgw_mov.Item(8, I).Value)), 2)
                SOLES = Math.Round(Decimal.Parse((dgw_mov.Item(27, I).Value)), 2) 'Math.Round(Decimal.Multiply(Decimal.Parse((dgw_mov.Item(8, I).Value)), Decimal.Parse((dgw_mov.Item(10, I).Value))), 2)
            End If
            If (dgw_mov.Item(26, I).Value.ToString = "1") Then
                VAR_PRO00 = "0"
            Else
                VAR_PRO00 = dgw_mov.Item(21, I).Value.ToString
            End If
            DT_DET.Rows.Add(AÑO, MES, COD_AUX, COD_COMP, txt_nro_comp.Text, (dgw_mov.Item(1, I).Value), (dgw_mov.Item(2, I).Value), (dgw_mov.Item(4, I).Value), (dgw_mov.Item(6, I).Value), (I + 1).ToString("0000"), (dgw_mov.Item(3, I).Value), (dgw_mov.Item(22, I).Value), (dgw_mov.Item(23, I).Value), (dgw_mov.Item(24, I).Value), (dgw_mov.Item(11, I).Value), (dgw_mov.Item(12, I).Value), SOLES, DOLARES, (dgw_mov.Item(7, I).Value), (dgw_mov.Item(9, I).Value), (dgw_mov.Item(10, I).Value), (dgw_mov.Item(13, I).Value), (dgw_mov.Item(14, I).Value), (dgw_mov.Item(15, I).Value), " ", (dgw_mov.Item(17, I).Value), "0", "", "", "0", VAR_PRO00, "0", dgw_mov.Item(26, I).Value.ToString, COD_MP, txt_nro_mp.Text, dtp_mp.Value.Date, (dgw_mov.Item(25, I).Value), "", VAR_PRO00, dtp_com.Value, 0, NOMBRE_PC)
            I += 1
        Loop
        Try
            con.Open()
        ...
        DT_RET.Rows.Clear()
        For I = 0 To CONT
            DT_RET.Rows.Add(COD_AUX, COD_COMP, txt_nro_comp.Text, AÑO, MES, (I + 1).ToString("0000"), DGW_RET2.Item(1, I).Value, DGW_RET2.Item(2, I).Value, DGW_RET2.Item(3, I).Value, DGW_RET2.Item(4, I).Value, DGW_RET2.Item(5, I).Value, DGW_RET2.Item(6, I).Value, DGW_RET2.Item(7, I).Value, DGW_RET2.Item(8, I).Value, NOMBRE_PC, DGW_RET2.Item(9, I).Value, txt_cod1.Text, DGW_RET2.Item(11, I).Value, DGW_RET2.Item(12, I).Value, DGW_RET2.Item(13, I).Value, DGW_RET2.Item(14, I).Value)
        Next
        Try
            con.Open()
            BULK.DestinationTableName = "DBO.T_RETENCIONES2"
        ...
        End If
        Try
            Dim comand1 As New SqlCommand("INSERTAR_I_CON_CXP_RETENCION", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            ...
            comand1.Parameters.Add("@NRO_MP", SqlDbType.VarChar).Value = txt_nro_mp.Text
            comand1.Parameters.Add("@TIPO_CAMBIO", SqlDbType.Decimal).Value = txt_cambio_pago.Text
            comand1.Parameters.Add("@FECHA_MP", SqlDbType.DateTime).Value = dtp_mp.Value.Date
            comand1.Parameters.Add("@STATUS_RE", SqlDbType.Int).Value = IIf(chkStatusRetencionElectronica.Checked = True, 1, 0)
            con.Open()
    ...
    Sub LIMPIAR_COMPROBANTE()
        ...
        'DEFAULTS
        COD_RET = obj.DIR_TABLA_DESC("DOCRET", "TDEFA") : cbo_ret.Text = obj.DESC_DOC(COD_RET)

        If cbo_ret.Text = "COMPROBANTE DE RETENCION" And RetencionElectronica = True Then
            txt_nro_ret.Text = "R"
            chkStatusRetencionElectronica.Checked = True
        Else
            txt_nro_ret.Clear()
        ...
        COD_MONEDA = obj.DIR_TABLA_DESC("MONRET", "TDEFA") : cbo_moneda_pago.Text = obj.DESC_MONEDA(COD_MONEDA)
        txt_cod_cta_pago.Text = obj.DIR_TABLA_DESC("CTARET", "TDEFA") : txt_desc_cta_pago.Text = obj.DESC_CUENTA(txt_cod_cta_pago.Text, AÑO)
    End Sub
    ...
    Function MODIFICAR_TODO() As String
        ...
        Do While (I <= CONT)
            ...
            Else
                VAR_PRO00 = dgw_mov.Item(21, I).Value.ToString
            End If
            DT_DET.Rows.Add(AÑO, MES, COD_AUX, COD_COMP, txt_nro_comp.Text, (dgw_mov.Item(1, I).Value), (dgw_mov.Item(2, I).Value), (dgw_mov.Item(4, I).Value), (dgw_mov.Item(6, I).Value), (I + 1).ToString("0000"), (dgw_mov.Item(3, I).Value), (dgw_mov.Item(22, I).Value), (dgw_mov.Item(23, I).Value), (dgw_mov.Item(24, I).Value), (dgw_mov.Item(11, I).Value), (dgw_mov.Item(12, I).Value), SOLES, DOLARES, (dgw_mov.Item(7, I).Value), (dgw_mov.Item(9, I).Value), (dgw_mov.Item(10, I).Value), (dgw_mov.Item(13, I).Value), (dgw_mov.Item(14, I).Value), (dgw_mov.Item(15, I).Value), " ", (dgw_mov.Item(17, I).Value), "0", "", "", "0", VAR_PRO00, "0", dgw_mov.Item(26, I).Value.ToString, COD_MP, txt_nro_mp.Text, (dgw_mov.Item(3, I).Value), (dgw_mov.Item(25, I).Value), "", VAR_PRO00, dtp_com.Value, 0, NOMBRE_PC) '  (dgw_mov.Item(0, I).Value), dgw_mov.Item(23, I).Value.ToString
            I += 1
        Loop
        Try
            con.Open()
        ...
        For I = 0 To CONT
            ...
            DT_RET.Rows.Add(COD_AUX, COD_COMP, txt_nro_comp.Text, AÑO, MES, (I + 1).ToString("0000"), DGW_RET2.Item(1, I).Value, DGW_RET2.Item(2, I).Value, DGW_RET2.Item(3, I).Value, DGW_RET2.Item(4, I).Value, DGW_RET2.Item(5, I).Value, DGW_RET2.Item(6, I).Value, DGW_RET2.Item(7, I).Value, (DGW_RET2.Item(8, I).Value), NOMBRE_PC, DGW_RET2.Item(9, I).Value, txt_cod1.Text, DGW_RET2.Item(11, I).Value, DGW_RET2.Item(12, I).Value, DGW_RET2.Item(13, I).Value, DGW_RET2.Item(14, I).Value) 'si pasa algo volver al 11
        ...
        End If
        Try
            Dim comand1 As New SqlCommand("MODIFICAR_I_CON_CXP_RETENCION", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
    ...
    Sub PERSONAS()
        Try
            ...
            DGW_PER_anul.DataSource = obj.MOSTRAR_PERSONAS_PAGAR
    ...
    Private Sub btn_ref0_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_ref0.Click
        ...
        If OFR_RET.DialogResult = Windows.Forms.DialogResult.OK Then
            ValorSoles = OFR_RET.txt_total2.Text
    ...
    Sub CARGAR_DIALOG()
        ...
        For I = 0 To CONT
            If DGW_RET2.Item(1, I).Value = COD_DOC And DGW_RET2.Item(2, I).Value = txt_nro_doc.Text Then
                ...
                End If
                OFR_RET.DGW_CAB.Rows.Add(DGW_RET2.Item(4, I).Value, DGW_RET2.Item(5, I).Value, DGW_RET2.Item(6, I).Value, DGW_RET2.Item(11, I).Value, IMP, DGW_RET2.Item(8, I).Value, DGW_RET2.Item(7, I).Value, DGW_RET2.Item(9, I).Value, "", "", DGW_RET2.Item(14, I).Value)
            End If
        Next
        ...
        With OFR_RET.DGW_CAB
            ...
            For I = 0 To CONT
                DGW_RET2.Rows.Add(ITEM0, COD_DOC, txt_nro_doc.Text, dtp_ret.Value.Date, .Item(0, I).Value, _
                    ...
                    txt_cod1.Text, .Item(3, I).Value, .Item(8, I).Value, .Item(9, I).Value, .Item(10, I).Value)
            Next
        End With
    ...
    Sub LIMPIAR_ANULADAS()
        ...
        '----------------------------------------------------------------------
        'DEFAULTS
        COD_RET_ANUL = obj.DIR_TABLA_DESC("DOCRET", "TDEFA") : cbo_ret_anul.Text = obj.DESC_DOC(COD_RET_ANUL)
        COD_MONEDA_ANUL = obj.DIR_TABLA_DESC("MONRET", "TDEFA") : cbo_moneda_pago_anul.Text = obj.DESC_MONEDA(COD_MONEDA_ANUL)
    ...
    Function INSERTAR_TODO_ANULADOS() As String
        Dim ESTADO As String = "FALLO"
        Try
            Dim comand1 As New SqlCommand("INSERTAR_I_CON_CXP_RETENCION_ANUL", con)
            comand1.CommandType = CommandType.StoredProcedure
    ...
    Function MODIFICAR_TODO_ANULADOS() As String
        Dim ESTADO As String = "FALLO"
        Try
            Dim comand1 As New SqlCommand("MODIFICAR_I_CON_CXP_RETENCION_ANUL", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX_ANUL
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP_ANUL
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp_anul.Text
    ...
    Private Sub btn_imprimir_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_imprimir.Click
        If chkStatusRetencionElectronica.Checked Then
            ...
            Dim archivo As String = RutaPdf + "\" + txt_nro_ret.Text + "-20" + ".pdf"

            If System.IO.File.Exists(archivo) Then
                MostrarPdf(txt_nro_ret.Text, RutaPdf)
            Else
                CrearPdf()
                MostrarPdf(txt_nro_ret.Text, RutaPdf)
            End If
        ...
        Else
            ...
            REP.TipoImpresion = obj.DIR_TABLA_DESC("IMPRET", "TDEFA")
            REP.Ruta = obj.DIR_TABLA_DESC("FILPDF", "TDEFA")
            Dim rutaLogo As String = obj.DIR_TABLA_DESC("IMPLOG", "TDEFA")
            Dim mensaje As String = obj.HALLAR_MENSAJE("RET")
            Dim NROAUT As String = obj.DIR_TABLA_DESC("RETAUT", "TDEFA")
            StatusRetencionElectronica = obj.DIR_TABLA_DESC("R_ELEC", "TDEFA")
            '-------------------------------------------------------------
            If StatusRetencionElectronica = "SI" Then
                REP.CREAR_REPORTE(dtp_ret.Value, txt_desc_per.Text, txt_nro_doc_per.Text, txt_cambio_pago.Text, Decimal.Parse(SUM2), _
                Decimal.Parse(SUM), L, txt_nro_ret.Text, rutaLogo, obj.HALLAR_DIRECCION_PERSONA(txt_cod1.Text), mensaje, "", "", "", "", RET)
            Else
                REP.CREAR_REPORTE(dtp_ret.Value, txt_desc_per.Text, txt_nro_doc_per.Text, txt_cambio_pago.Text, Decimal.Parse(SUM2), _
                            Decimal.Parse(SUM), L, txt_nro_ret.Text, rutaLogo, obj.HALLAR_DIRECCION_PERSONA(txt_cod1.Text), mensaje, NROAUT.Substring(0, 10), NROAUT.Substring(10, 3), NROAUT.Substring(13, 5), NROAUT.Substring(18, 5), RET)
            End If
            'REP.ExportarPDF(txt_nro_ret.Text)
            REP.ShowDialog()
            'REP.CREAR_REPORTE(dtp_ret.Value, txt_desc_per.Text, txt_nro_doc_per.Text, txt_cambio_pago.Text, Decimal.Parse(SUM2), _
            '    Decimal.Parse(SUM), L, txt_nro_ret.Text, rutaLogo, obj.HALLAR_DIRECCION_PERSONA(txt_cod1.Text), mensaje, NROAUT.Substring(0, 10), NROAUT.Substring(10, 3), NROAUT.Substring(13, 5), NROAUT.Substring(18, 5))
            'REP.ShowDialog()
        End If
    End Sub
    ...
    Sub LLENAR_DATASET()
        ...
        If RetencionElectronica Then
            ListaCopia.Add("PROVEEDOR")
        Else
            ListaCopia.Add("PROVEEDOR")
            ListaCopia.Add("EMISOR - AGENTE DE RETENCIÓN")
            ListaCopia.Add("SUNAT")
            ListaCopia.Add("CONTROL ADMINISTRATIVO")
        End If
        Dim CodMoneda As String = String.Empty
        For I = 0 To CONT
            '-------------------------------------------------------------------
            ...
            End If
            '-------------------------------------------------------------------
            Dim BASE_I, IMPORTE, BASE_IMP As Decimal
            If DGW_RET2.Item(9, I).Value.ToString = "H" Then
                'BASE_I = DGW_RET2.Item(8, I).Value * -1
                If RetencionElectronica Then
                    BASE_I = DGW_RET2.Item(12, I).Value * -1
                Else
                    BASE_I = DGW_RET2.Item(8, I).Value * -1
                End If
                IMPORTE = DGW_RET2.Item(7, I).Value * -1
                BASE_IMP = DGW_RET2.Item(8, I).Value * -1
            Else
                'BASE_I = DGW_RET2.Item(8, I).Value
                'IMPORTE = DGW_RET2.Item(7, I).Value
                If RetencionElectronica Then
                    BASE_I = DGW_RET2.Item(12, I).Value
                Else
                    BASE_I = DGW_RET2.Item(8, I).Value
                End If
                IMPORTE = DGW_RET2.Item(7, I).Value
                BASE_IMP = DGW_RET2.Item(8, I).Value
                If DGW_RET2.Item(11, I).Value = "D" Then
                    CodMoneda = "USD"
                ElseIf DGW_RET2.Item(11, I).Value = "S" Then
                    CodMoneda = "PEN"
                Else
                    CodMoneda = ""
                End If
            End If
            If obj.DESC_DOC(DGW_RET2.Item(4, I).Value.ToString) = "NOTA DE CREDITO" Then
                Documento = "N. CREDITO"
            ElseIf obj.DESC_DOC(DGW_RET2.Item(4, I).Value.ToString) = "NOTA DE DEBITO" Then
                Documento = "N. DEBITO"
            Else
                Documento = obj.DESC_DOC(DGW_RET2.Item(4, I).Value.ToString)
            End If
            If obj.DIR_TABLA_DESC("IMPRET", "TDEFA") = "V" Then
                For Each s As String In ListaCopia
                    REP.llenar_dt(Documento, SERIE.ToString, CORRELATIVO.ToString, Date.Parse(DGW_RET2.Item(6, I).Value), BASE_I, IMPORTE, DGW_RET2.Item(9, I).Value, s, BASE_IMP - IMPORTE, CodMoneda, DGW_RET2.Item(14, I).Value.ToString)
                Next
            Else
                REP.llenar_dt(Documento, SERIE.ToString, CORRELATIVO.ToString, Date.Parse(DGW_RET2.Item(6, I).Value), BASE_I, IMPORTE, DGW_RET2.Item(9, I).Value, "", 0, CodMoneda, DGW_RET2.Item(14, I).Value.ToString)
            End If
        Next
    End Sub
    ...
    Private Sub cbo_ret_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_ret.SelectedIndexChanged
        If cbo_ret.Text = "COMPROBANTE DE RETENCION" And RetencionElectronica = True Then
            txt_nro_ret.Text = txt_nro_ret.Text.Insert(0, "R")
        Else
            txt_nro_ret.Text = txt_nro_ret.Text.Replace("R", "")
        End If
    End Sub
    Private Sub CrearPdf()
        ...
        REP.ExportarPDF(txt_nro_ret.Text)
    End Sub
    Private Sub MostrarPdf(ByVal NombreArchivo As String, ByVal RutaPdf As String)
        If RutaPdf.Trim = "" Then
            MessageBox.Show("La ruta de los pdf no esta definida", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
        Dim archivo As String = RutaPdf + "\" + NombreArchivo + "-20" + ".pdf"
        Using frm As New Form
            Dim wb As New WebBrowser
            'File.WriteAllBytes(archivo, respuesta.pdfFile)
