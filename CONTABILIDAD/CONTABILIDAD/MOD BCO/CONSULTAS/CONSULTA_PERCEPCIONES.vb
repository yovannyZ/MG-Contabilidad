...
Public Class CONSULTA_PERCEPCIONES
#Region "Constructor"
    Private Shared _instancia As CONSULTA_PERCEPCIONES
    Public Shared Function ObtenerInstancia() As CONSULTA_PERCEPCIONES
        If _instancia Is Nothing OrElse _instancia.IsDisposed Then
            _instancia = New CONSULTA_PERCEPCIONES
        End If
...
#End Region
    ...
    Sub CARGAR_CONSULTA_MOV()
        dgw3.Rows.Clear()
        Try
            Dim command As New SqlCommand("[MOSTRAR_CONSULTA_PERCEPCION_DET2]", con)
            command.CommandType = (CommandType.StoredProcedure)
            command.Parameters.Add("@COD_PER", SqlDbType.VarChar).Value = (txt_cod2.Text)
            command.Parameters.Add("@NRO_PERC", SqlDbType.VarChar).Value = (txt_nro.Text)
            ...
            Do While reader.Read
                dgw3.Rows.Add(reader.GetValue(0), reader.GetValue(1), reader.GetValue(2), reader.GetValue(3), reader.GetValue(4), reader.GetValue(5), Format(reader.GetValue(6), "0.00"), reader.GetValue(7), reader.GetValue(8), reader.GetValue(9), reader.GetValue(10), reader.GetValue(11), reader.GetValue(12))
            Loop
            con.Close()
    ...
    Sub CARGAR_CONSULTA_PER()
        DGW1.Rows.Clear()
        Try
            Dim command As New SqlCommand("[MOSTRAR_CONSULTA_PERCEPCION]", con)
            command.CommandType = (CommandType.StoredProcedure)
    ...
    Sub CARGAR_DET(ByVal NRO_RET001 As Object)
        dgw11.Rows.Clear()
        Try
            Dim command As New SqlCommand("[MOSTRAR_CONSULTA_PERCEPCION_DET]", con)
            command.CommandType = (CommandType.StoredProcedure)
            command.Parameters.Add("@COD_PER", SqlDbType.VarChar).Value = (TXT_COD1.Text)
            command.Parameters.Add("@NRO_PERC", SqlDbType.VarChar).Value = NRO_RET001
            con.Open()
            command.ExecuteNonQuery()
            Dim reader As SqlDataReader = command.ExecuteReader
            Do While reader.Read
                dgw11.Rows.Add(reader.GetValue(0), reader.GetValue(1), reader.GetValue(2), reader.GetValue(3), reader.GetValue(4), reader.GetValue(5), Format(reader.GetValue(6), "0.00"), reader.GetValue(7), reader.GetValue(8), reader.GetValue(9))
            Loop
            con.Close()
        Catch ex As Exception
            con.Close()
        ...
        End Try
    End Sub
    Sub CARGAR_PERCEPCIONES(ByVal MES10 As Object)
        Try
            Dim command As New SqlCommand("MOSTRAR_NRO_PERCEPCION_AÑO_MES", con)
            command.CommandType = (CommandType.StoredProcedure)
            command.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = CBO_AÑO.Text
    ...
    Sub CARGAR_PERSONAS()
        Try
            Dim command As New SqlCommand("MOSTRAR_PERSONAS_XCOBRAR", con)
            Dim adapter As New SqlDataAdapter(command)
