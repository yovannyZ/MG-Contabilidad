...
Public Class GCO_TRANSF_HON
                    ...
                    If dgw1.Item(10, I).Value.ToString = "True" Then
                        ...
                        Else
                            ...
                            If VALIDAR_CUENTA_ORDEN Then
                                INSERTAR_DOCUMENTO()
                                GENERAR_AUTO()
                                If (((VERIFICAR_PERSONA AndAlso VERIFICAR_CUENTA) AndAlso (VERIFICAR_ORDEN AndAlso VERIFICAR_CC)) AndAlso ((VERIFICAR_CONTROL AndAlso VERIFICAR_PROYECTO) AndAlso VERIFICAR_AUTO)) Then
            ...
            Else
                ...
                Do While (I <= CONT)
                    If dgw1.Item(10, I).Value.ToString = "True" Then
                        ...
                        GENERAR_AUTO()
                        VERIFICAR_PERSONA()
                        If (((VERIFICAR_CUENTA = False OrElse VERIFICAR_ORDEN = False) OrElse (VERIFICAR_CC = False OrElse VERIFICAR_CONTROL = False)) OrElse (VERIFICAR_PROYECTO = False OrElse VERIFICAR_AUTO = False)) Then
                            Return
                        End If
                    End If
                    I += 1
    ...
    Private Sub btn_sgt1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_sgt1.Click
        ...
        txt_letra1.Focus()
        Dim CONT0 As Integer = (dgw1.RowCount - 1)
        Dim i As Integer = Integer.Parse(fila1)
        Do While (i <= CONT0)
            Dim CONT1 As Integer = Strings.Len(dgw1.Item(7, i).Value.ToString)
            Dim f As Integer = 1
            Do While (f <= CONT1)
    ...
    Private Sub btn_sgt2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_sgt2.Click
        ...
        txt_letra2.Focus()
        Dim CONT0 As Integer = (dgw2.RowCount - 1)
        Dim i As Integer = Integer.Parse(fila2)
        Do While (i <= CONT0)
            Dim CONT1 As Integer = Strings.Len(dgw2.Item(7, i).Value.ToString)
            Dim f As Integer = 1
            Do While (f <= CONT1)
    ...
    Public Sub CARGAR_CERRADAS()
        Try
            Dim CMD As New SqlCommand("COI_MOSTRAR_I_HON_CERRADA_COI", CON_GCO)
    ...
    Public Sub CARGAR_PENDIENTES()
        ...
        End Try
        Try
            Dim CMD As New SqlCommand("COI_MOSTRAR_I_HON_PTE_COI", CON_GCO)
            CMD.CommandType = CommandType.StoredProcedure
            CMD.Parameters.Add("@TIPO_USU", SqlDbType.Char).Value = TIPO_USU
            CMD.Parameters.Add("@COD_USU", SqlDbType.Char).Value = COD_USU
            CMD.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
    ...
    Public Sub DOC_TRANSFERIR(ByVal FIla As Integer)
        dgw_mov1.Rows.Clear()
        Try
        ...
        Do While (I <= CONT)
            If (dgw_mov1.Item(1, I).Value.ToString <> "") Then
                dgw_mov1.Item(11, I).Value = OBJ.HALLAR_ORDEN_CUENTA("B", AÑO, "H", dgw_mov1.Item(1, I).Value.ToString.Substring(0, 3))
            End If
            I += 1
        Loop
    End Sub
    ...
    Private Sub GCO_TRANSFERENCIA_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        CUENTA_IGV = OBJ.GCO_DIR_TABLA_DESC("REN", "TCTAS", CON_GCO)
        ORDEN_IGV = OBJ.HALLAR_ORDEN_CUENTA("R", AÑO, "H", CUENTA_IGV.Substring(0, 3))
        REN0 = Decimal.Parse(OBJ.IMPUESTO("REN"))
        CARGAR_CERRADAS()
    End Sub

    Public Sub GENERAR_AUTO()
        ...
        Do While (j <= cont0)
            ...
            If (var14.Length = 8) Then
                ...
                End Select
                dgw_det_01.Rows.Add(New Object() {var0, var14, var2, var3, var4, cod_enlace, var6, var7, "", "", "", "", var12, var13, "", "", "", "", "", var19, var20, var21, var22, var23, "1", "0", " ", "0", "0", " ", " ", Now.Date, 0, " ", 0, 0, "0", "0", "0", var24})
    ...
    Public Sub INSERTAR_DOCUMENTO()
        ...
        End If
        '-------------------------------------------------------
        dgw_doc_01.Rows.Add(New Object() {COD_AUX, COD_COMP, txt_nro_comp.Text, COD_DOC, NRO_DOC, COD_PER, DESC_PER, DOC_PER, dtp_com.Value, FECHA_DOC, cant01, cant02, cant03, cant04, cant05, cant06, cant07, cant08, cant09, cant10, COD_MONEDA, TCAMBIO, cte_03, COD_DH_DOC, "", 0, FECHA_DOC, "", "", FECHA_DOC, FECHA_DOC, "0", 0, imp_s_t})
        Dim j As Integer = 0
        j = 0
        Do While (j <= CONT)
        ...
        Else
            ...
            imp_t00 = imp_d_t
        End If
        Dim variable1 As Object() = New Object() {(CONT + 2).ToString("0000"), CUENTA_IGV, DESC_PER, Math.Round(imp_s_igv, 2), Math.Round(imp_d_igv, 2), DH_IGV, COD_MONEDA, TCAMBIO, " ", " ", " ", ORDEN_IGV, FECHA_VEN, 0, " ", " ", " ", " ", " ", FECHA_DOC, COD_DOC, NRO_DOC, COD_PER, DOC_PER, 0, "R", "", "", "0", " ", " ", FECHA_DOC, 0, " ", 0, imp_igv00, "0", "0", "0", DESC_PER}
        dgw_det_01.Rows.Add(variable1)
        variable1 = New Object(40 - 1) {}
        variable1(0) = (CONT + 3).ToString("0000")
        variable1(1) = CUENTA_TOTAL
        variable1(2) = DESC_PER
        variable1(3) = Math.Round(imp_s_t, 2)
        variable1(4) = Math.Round(imp_d_t, 2)
        variable1(5) = DH_T
        variable1(6) = COD_MONEDA
        variable1(7) = TCAMBIO
        variable1(8) = " "
        variable1(9) = " "
        variable1(10) = " "
        variable1(11) = ORDEN_TOTAL
        variable1(12) = FECHA_VEN
        variable1(13) = 0
        variable1(14) = " "
        variable1(15) = " "
        variable1(16) = " "
        variable1(17) = " "
        variable1(18) = " "
        variable1(19) = FECHA_DOC
        variable1(20) = COD_DOC
        variable1(21) = NRO_DOC
        variable1(22) = COD_PER
        variable1(23) = DOC_PER
        variable1(24) = 0
        variable1(25) = "T"
        variable1(26) = ""
        variable1(27) = ""
        ...
        variable1(29) = " "
        variable1(30) = " "
        variable1(31) = FECHA_DOC
        variable1(32) = 0
        variable1(33) = " "
        variable1(34) = 0
        variable1(35) = imp_t00
        variable1(36) = "0"
        variable1(37) = "0"
        variable1(38) = "0"
        variable1(39) = DESC_PER
        dgw_det_01.Rows.Add(variable1)
    End Sub
    ...
    Public Function VALIDAR_CUENTA_ORDEN() As Boolean
        ...
        I = 0
        Do While (I <= CONT)
            If ((dgw_mov1.Item(1, I).Value.ToString <> "") AndAlso (OBJ.HALLAR_ORDEN_CUENTA("B", AÑO, "H", dgw_mov1.Item(1, I).Value.ToString.Substring(0, 3)) = "0")) Then
    ...
    Public Function VERIFICAR_AUTO() As Boolean
        ...
        Do While (I <= CONT)
            If (((dgw_det_01.Item(25, I).Value.ToString = "B") And (dgw_det_01.Item(14, I).Value.ToString.Trim <> "")) And (dgw_det_01.Item(15, I).Value.ToString.Trim <> "")) Then
                ...
                Try
                    ...
                    comand1.Parameters.Add("@CUENTA_ENLACE", SqlDbType.VarChar).Value = dgw_det_01.Item(14, I).Value.ToString
                    comand1.Parameters.Add("@CUENTA_DESTINO", SqlDbType.VarChar).Value = dgw_det_01.Item(15, I).Value.ToString
        ...
        If (SB.Length = 0) Then
            Return True
        End If
        MessageBox.Show(("No existen en el Maestro de Automaticas  Cuenta : Destino/Enlace : " & vbCrLf & " " & CTA0 & " : " & SB.ToString), "No se puede Transferir", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        Return False
    End Function
    ...
    Public Function VERIFICAR_CONTROL() As Boolean
        ...
        If (SB.Length = 0) Then
            Return True
        End If
        MessageBox.Show(("No existen en el Maestro de Control : " & vbCrLf & SB.ToString), "No se puede Transferir", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        Return False
    End Function
                ...
                If (dgw_det_01.Item(14, I).Value.ToString.Trim = "") Then
                    ...
                    Try
                        ...
                        Do While Rs4.Read
                            ...
                            SB.AppendLine((String.Concat(String.Concat(String.Concat(String.Concat(Rs4.GetValue(0), " "), dgw_det_01.Item(2, I).Value.ToString), "  -  "), dgw_det_01.Item(39, I).Value.ToString)))
                        Loop
                        con.Close()
                    Catch ex As Exception
    ...
    Public Function VERIFICAR_ORDEN() As Boolean
        ...
        Do While (I <= CONT)
            ...
            If (((TIPO_ORDEN0 = "B") Or (TIPO_ORDEN0 = "R")) AndAlso (dgw_det_01.Item(11, I).Value.ToString = "")) Then
