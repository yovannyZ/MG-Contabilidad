...
Imports System.Text
Public Class CXC_INI_TRANSFERENCIA
        ...
        Do While (I <= CONT)
            If dgw1.Item(11, I).Value = True Then
                If VERIFICAR_CUENTA(I) Then
                    If OBJ.VERIFICAR_PCTAS_COBRAR(dgw1.Item(2, I).Value.ToString, dgw1.Item(3, I).Value.ToString, dgw1.Item(4, I).Value.ToString, dgw1.Item(6, I).Value.ToString, dgw1.Item(10, I).Value.ToString) = False Then
    ...
    Private Sub btn_aceptar2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar2.Click
        ...
        Do While (I <= CONT)
            If dgw2.Item(11, I).Value = True Then
                Dim CTA0 As String = OBJ.BUSCAR_CUENTA_PCXC(Integer.Parse(dgw2.Item(0, I).Value.ToString).ToString("00"), dgw2.Item(4, I).Value.ToString, dgw2.Item(2, I).Value.ToString, dgw2.Item(3, I).Value.ToString)
                If ((CTA0 <> "") And OBJ.VERIFICAR_ELIMINAR_PCTAS_COBRAR(dgw2.Item(2, I).Value.ToString, dgw2.Item(3, I).Value.ToString, dgw2.Item(4, I).Value.ToString, dgw2.Item(6, I).Value.ToString, CTA0) = False) Then
    ...
    Sub CARGAR_CERRADAS()
        If (MES = "00") Then
            Try
                Dim CMD As New SqlCommand("COI_MOSTRAR_CXC_INI_CERRADA_00", CON_GCO)
        ...
        Else
            Try
                Dim CMD As New SqlCommand("COI_MOSTRAR_CXC_INI_CERRADA", CON_GCO)
                CMD.CommandType = (CommandType.StoredProcedure)
                CMD.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
    ...
    Sub CARGAR_PENDIENTES()
        If (MES = "00") Then
            Try
                Dim CMD As New SqlCommand("COI_MOSTRAR_CXC_INI_PTE_00", CON_GCO)
                CMD.CommandType = (CommandType.StoredProcedure)
                CMD.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
                Dim ADAP As New SqlDataAdapter(CMD)
                Dim DT As New DataTable("SD")
    ...
    Function DESTRANSFERIR_COI(ByVal FILA0 As Integer) As String
        ...
        Try
            Dim comand1 As New SqlCommand("GCO_REGRESAR_CXC_INI", con)
    ...
    Function GRABAR_COI(ByVal FILA0 As Integer) As String
        ...
        Dim estado As String = "FALLO"
        Try
            Dim comand1 As New SqlCommand("GCO_TRANSFERIR_CXC_INI", con)
            comand1.CommandType = (CommandType.StoredProcedure)
    ...
    Sub STATUS_COI(ByVal FILA0 As Integer)
        Try
            Dim pro_02 As New SqlCommand("COI_CXC_STATUS", CON_GCO)
    ...
    Sub STATUS_COI2(ByVal FILA0 As Integer)
        Try
            Dim pro_02 As New SqlCommand("COI_CXC_STATUS2", CON_GCO)
            pro_02.CommandType = (CommandType.StoredProcedure)
    ...
    Private Sub txt_letra1_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txt_letra1.TextChanged
        ...
        ElseIf ch_per1.Checked Then
            txt_letra1.Focus()
            'Dim VB$t_i4$L1 As Integer = (dgw1.RowCount - 1)
            i = 0
            Do While (i <= (dgw1.RowCount - 1))
    ...
    Private Sub txt_letra2_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txt_letra2.TextChanged
        ...
        ElseIf ch_per2.Checked Then
            txt_letra2.Focus()
            'Dim VB$t_i4$L1 As Integer = (dgw2.RowCount - 1)
            i = 0
            Do While (i <= (dgw2.RowCount - 1))
    ...
    Function VERIFICAR_CUENTA(ByVal FILA0 As Object) As Boolean
        ...
        Return False
    End Function
    Private Sub CXC_INI_TRANSFERENCIA_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        main(39) = 0
    End Sub
    Private Sub CXC_INI_TRANSFERENCIA_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
