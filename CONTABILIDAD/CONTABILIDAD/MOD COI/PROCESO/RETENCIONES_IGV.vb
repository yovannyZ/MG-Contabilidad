...
Public Class RETENCIONES_IGV
   ...
   Sub CARGAR_AÑO()
    ...
    Sub LLENAR_DGW()
        ...
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_TEXTO_RETENCIONES", con)
            ...
            Do While Rs3.Read
                Dim SERIE_RET, NRO_RET, NRO_DOC_RET As String
                Dim TAMAÑO, GUION As Integer
                '------------------------------------------------------------------
                NRO_RET = Rs3.GetValue(5)
                GUION = NRO_RET.LastIndexOf("-")
                TAMAÑO = NRO_RET.Length
                If TAMAÑO <> 0 Then
                    SERIE_RET = NRO_RET.Substring(0, GUION)
                    NRO_DOC_RET = NRO_RET.Substring(GUION + 1, TAMAÑO - (GUION) - 1)
                Else
                    SERIE_RET = " "
                    NRO_DOC_RET = " "
                End If
                '------------------------------------------------------------------
                Dim SERIE_DOC, DOC_RET, DOC_RET2 As String
                Dim TAMAÑO2, GUION2 As Integer
                DOC_RET = Rs3.GetValue(9)
                GUION2 = DOC_RET.LastIndexOf("-")
                TAMAÑO2 = DOC_RET.Length
                If TAMAÑO2 <> 0 Then
                    SERIE_DOC = DOC_RET.Substring(0, GUION2)
                    DOC_RET2 = DOC_RET.Substring(GUION2 + 1, TAMAÑO2 - (GUION2) - 1)
                Else
                    SERIE_DOC = " "
                    DOC_RET2 = " "
                End If
                '------------------------------------------------------------------
                DGW_DETALLE.Rows.Add(HALLAR_ITEM, (Rs3.GetValue(0)), Rs3.GetValue(1), Rs3.GetValue(2), Rs3.GetValue(3), Rs3.GetValue(4), SERIE_RET, NRO_DOC_RET, Rs3.GetValue(6).ToString.Substring(0, 10), Rs3.GetValue(7), Rs3.GetValue(8), SERIE_DOC, DOC_RET2, Rs3.GetValue(10).ToString.Substring(0, 10), Rs3.GetValue(11), Rs3.GetValue(12))
            Loop
            con.Close()
        Catch ex As Exception
            con.Close()
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Me.sfdArchivos.FileName = "0626" & txt_ruc.Text & cbo_año.Text & OBJ.COD_MES(cbo_mes.Text)
    ...
    Sub CARGAR_ESTRUCTURA()
        ...
        For i = 0 To cont - 1
            SB.AppendLine(Trim(DGW_DETALLE.Item(1, i).Value) & "|" & Trim(DGW_DETALLE.Item(2, i).Value) & "|" & Trim(DGW_DETALLE.Item(3, i).Value) & "|" & Trim(DGW_DETALLE.Item(4, i).Value) & "|" & Trim(DGW_DETALLE.Item(5, i).Value) & "|" & Trim(DGW_DETALLE.Item(6, i).Value) & "|" & Trim(DGW_DETALLE.Item(7, i).Value) & "|" & Trim(DGW_DETALLE.Item(8, i).Value) & "|" & Trim(DGW_DETALLE.Item(9, i).Value) & "|" & Trim(DGW_DETALLE.Item(10, i).Value) & "|" & Trim(DGW_DETALLE.Item(11, i).Value) & "|" & Trim(DGW_DETALLE.Item(12, i).Value) & "|" & Trim(DGW_DETALLE.Item(13, i).Value) & "|" & OBJ.HACER_DECIMAL_PTO(Trim(DGW_DETALLE.Item(15, i).Value)) & "|")
        Next
        Try
