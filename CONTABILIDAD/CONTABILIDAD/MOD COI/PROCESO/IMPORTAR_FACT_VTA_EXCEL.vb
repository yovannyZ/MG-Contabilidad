...
Public Class IMPORTAR_FACT_VTA_EXCEL
    ...
    Private MontoPle As Decimal

    Private Sub Formato()
        DG.Columns(0).Width = 30 : DG.Columns(1).Width = 80
        DG.Columns(2).Width = 70 : DG.Columns(3).Visible = False
        DG.Columns(4).Width = 50 : DG.Columns(5).Width = 140
        DG.Columns(6).Width = 150 : DG.Columns(7).Width = 80
        DG.Columns(8).Width = 40 : DG.Columns(9).Width = 50
        DG.Columns(10).Width = 50 : DG.Columns(11).Width = 80
        DG.Columns(12).Width = 40 : DG.Columns(13).Width = 70
        ...
        DG.Columns(20).Width = 40 ': DG.Columns(21).Width = 70
        'DG.Columns(22).Width = 50 : DG.Columns(23).Width = 50
    End Sub

    Private Sub ValidacionesPle()
        Dim x As Integer
        'Dim SbMensajeValidaciones As New StringBuilder
        'Validamos que no hayan inconsistencias
        'SbMensajeValidaciones.Remove(0, SbMensajeValidaciones.Length)
        Dim frmVisorErrores As New Form
        Dim dgv As New DataGridView
        Dim col1 As New DataGridViewTextBoxColumn
        Dim col2 As New DataGridViewTextBoxColumn
        Dim col3 As New DataGridViewTextBoxColumn
        col1.HeaderText = "Doc" : col2.HeaderText = "Nro Doc" : col3.HeaderText = "Mensaje"
        ...
        frmVisorErrores.Text = "Registros con observaciones"
        For x = 0 To DG.Rows.Count - 1
            If DG.Item(10, x).Value > MontoPle AndAlso _
            (DG.Item(5, x).Value.ToString.Length = 0 Or DG.Item(6, x).Value.ToString.Length = 0) Then
                'SbMensaje.AppendLine("El doc. " + DG.Item(1, x).Value.ToString + " no cumple con las condiciones" & vbCrLf)
                dgv.Rows.Add(DG.Item(0, x).Value.ToString, DG.Item(1, x).Value.ToString, "Verifique el Ruc, Nombre persona o código")
            End If
        Next
        'If SbMensaje.Length > 0 Then
        '    MessageBox.Show(SbMensaje.ToString, "Registros Inconsistentes", MessageBoxButtons.OK, MessageBoxIcon.Information)
        'End If
        If dgv.Rows.Count > 0 Then
            frmVisorErrores.Controls.Add(dgv)
            frmVisorErrores.MdiParent = Me.MdiParent
            frmVisorErrores.Show()
        End If
    End Sub
    ...
    Private Sub GRABAR_PCTAS_COBRAR()
        ...
        CMD.Parameters.Add("@COD_CUENTA", SqlDbType.Char).Value = COD_CC 'CUENTA_TOTAL
        ...
        Try
            ...
            For x = 0 To DG.Rows.Count - 1
                If DG.Item(11, x).Value Then
                    ...
                    DOC_PER = Trim(DG.Item(6, x).Value.ToString) '7
                    'COD_CLASE = DG.Item(2, x).Value.ToString
                    COD_MONEDA = DG.Item(7, x).Value.ToString '8
                    TCAMBIO = Decimal.Parse((DG.Item(9, x).Value)) '10
                    FECHA_VEN = FECHA_DOC
                    IMP_BASE = Decimal.Parse(DG.Item(10, x).Value.ToString) '11
                    'IMP_EXO = Decimal.Parse(DG.Item(14, x).Value.ToString)
                    IMP_IGV = Decimal.Parse((DG.Item(13, x).Value)) '16
                    IMP_TOTAL = Decimal.Parse(DG.Item(15, x).Value.ToString) '18
                    CUENTA_BASE = DG.Item(12, x).Value.ToString '13
                    'CUENTA_EXO = DG.Item(15, x).Value.ToString
                    CUENTA_IGV = DG.Item(14, x).Value.ToString '17
                    CUENTA_TOTAL = DG.Item(15, x).Value.ToString '19
                    COD_CC = DG.Item(16, x).Value.ToString '22
                    'TEMP
                    COD_C = DG.Item(16, x).Value.ToString
                    'ORDEN DE CUENTA
                    ORDEN_BASE = HALLAR_ORDEN_CUENTA("B", AÑO, "V", DG.Item(12, x).Value.ToString.Substring(0, 3)) '13
                    'ORDEN_EXONERADO = HALLAR_ORDEN_CUENTA("B", AÑO, "V", DG.Item(15, x).Value.ToString.Substring(0, 3))
                    ORDEN_IGV = HALLAR_ORDEN_CUENTA("I", AÑO, "V", DG.Item(14, x).Value.ToString.Substring(0, 3)) '17
                    ORDEN_TOTAL = HALLAR_ORDEN_CUENTA("T", AÑO, "V", DG.Item(16, x).Value.ToString.Substring(0, 3)) '19
                    ...
                    For d = 12 To 18 Step 2
                        ...
                        ElseIf d = 16 AndAlso IMP_IGV > 0 Then
                            ...
                            igv = 0
                            'If para la cuenta corriennte

                        ElseIf d = 18 AndAlso IMP_TOTAL > 0 Then
                            _Grabar = True
                            Cta = COD_C  'CUENTA TOTAL
                            COD_CC = ""
                ...
                If NRO_COMP = 10000 Then
                    MessageBox.Show("Ha llegado al tope de registros" + vbLf + " Cargue de nuevo y continue en otro Comprobante", "Comprobante", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit For
                End If
            Next
    ...
    Sub MostrarCodPersona()
        ...
        While I <= CONT
            With DG
                If Not (VERIFICAR_REG_VENTAS(.Item(0, I).Value, .Item(1, I).Value, .Item(4, I).Value, .Item(6, I).Value)) Then
                    DG.Rows.RemoveAt(I)
                    I = 0 : CONT = DG.RowCount - 1
                Else
                    I += 1
    ...
    '    Catch ex As Exception
    '        MessageBox.Show("Error al verificar el codigo de la persona" & Environment.NewLine & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    End Try
    '    Return COD
    'End Function
    ...
    Function VALIDAR_CUENTA_ORDEN(ByVal fila As Integer) As Boolean
        ...
        Try
            Dim i As Integer = 0
            Dim oB As String = ""
            For i = 12 To 16 Step 2  '13 To 19 Step 2
                If i = 12 Then
                    oB = "B"
                    'ElseIf i = 15 Then
                    '    oB = "B"
    ...
    Private Function CargarDatos() As DataSet
        ...
        Dim ruta As String = txt_Ifac.Text
        'Dim sb As New StringBuilder
        ...
        '    'cnn.Open()
        '    Dim sql As String = "SELECT CDDOC,NUDOC,FEDOC,A.CDVTA,NURUC,CDMON, " & _
        ...
        'End Using
        Try
            Using cnFactura As New OleDbConnection("Provider=Microsoft.ACE.Oledb.12.0;" & _
                                                           "Extended Properties='TEXT;HDR=Yes';" & _
                                                           "Data Source=" & ruta & ";" & _
                                                           "Extended Properties= Excel 12.0")
                '"Select CDDOC,NUDOC,FEDOC,F.CDVTA,CDCTE,DSCTE,NURUC,CDMON,NUDIA,TPCAM,TOB_I,CDC_B,TOIGV,CDC_I,(TOB_I+TOIGV) AS TOTAL,CDC_C,CDFAC,NUFAC,FEFAC,STANU from [IFAC10$] F inner join [MI_V01$] M on F.CDDOC=M.CDVTA"
                Dim sql As String = "Select CDDOC,NUDOC,FEDOC,F.CDVTA," & _
                                    "CDCTE,NURUC,CDMON,NUDIA,TPCAM,TOB_I," & _
                                    "CDC_B,TOIGV,CDC_I,(TOB_I+TOIGV) AS TOTAL, " & _
                                    "CDC_C,CDFAC,NUFAC,FEFAC,STANU,DSCTE from [IFAC10$] F inner join [MI_V01$] M on F.CDVTA=M.CDVTA"
                Dim Command As New OleDbCommand(sql, cnFactura)
                Dim da As New OleDbDataAdapter(Command)
                da.Fill(dt)

            End Using
        Catch ex As Exception
            MsgBox("El archivo no tiene las hojas especificadas" + Chr(13) + "O las hojas no tienen los campos especificados", MsgBoxStyle.Exclamation, "Error")
        End Try
        ...
        SB.AppendLine("[Cod.]=Case CDCTE when '00000' then '00000' else ISNULL(M.COD_PER,'') END,[Cliente]= Case when CDCTE='00000' AND ISNULL(STANU,'') = 'A' then 'ANULADO' WHEN CDCTE = '00000' AND TOB_I > @LIMITE THEN DSCTE when CDCTE='00000' AND ISNULL (STANU,'')<>'A' then (Select DESC_PER from MAESTRO_PERSONAS where COD_PER ='00000') else ISNULL(M.DESC_PER,'') END,[Ruc.]=ISNULL(NURUC,''), CDMON AS [M],")
        sb.AppendLine("NUDIA AS [Dia],CAST(TPCAM AS DECIMAL(13,3)) AS [T.c.],CAST(TOB_I AS DECIMAL(13,2)) AS [Importe],@OK [Ok],CDC_B AS [Cdc_b],")
        sb.AppendLine("CAST(TOIGV AS DECIMAL(13,2)) AS [Igv], CDC_I AS [Cdc_i],")
        SB.AppendLine("CAST((TOB_I+TOIGV)AS DECIMAL(13,2)) AS Total,CDC_C AS [Cdc_c],CDFAC AS [Doc.R.],NUFAC AS [Fac.],FEFAC AS [Fecha],STANU AS [Anu]")
        SB.AppendLine("FROM FACT_DBF LEFT JOIN MAESTRO_PERSONAS M ON M.NRO_DOC=NURUC AND M.NRO_DOC <> '' WHERE YEAR(FEDOC)=@FE_AÑO AND MONTH(FEDOC)=@FE_MES")
        SB.AppendLine("ORDER BY NUDOC")
        ...
        cmdt.Parameters.Add("@LIMITE", SqlDbType.Decimal).Value = MontoPle
    ...
    Private Sub GenerarContabilidad(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenera.Click
        If txtComprobante.Text = "9999" Then
            MessageBox.Show("La serie del comprobante llego a su tope" + vbLf + "Cambiar comprobate", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        If (cboAuxiliar.SelectedIndex = -1) Then
    ...
    Private Sub IMPORTAR_FACT_VTA_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Call CBO_AUXILIAR()
        MontoPle = IIf(OBJ.DIR_TABLA_DESC("IMPPLE", "TDEFA") = " ", 0, OBJ.DIR_TABLA_DESC("IMPPLE", "TDEFA"))
    End Sub

    Private Sub Btn_Consultar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Consultar.Click
        If (txt_Ifac.TextLength = 0) Then
    ...
    Private Sub DG_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG.CellClick
        Try
            If e.ColumnIndex = 11 Then
                If (DG.Item(4, e.RowIndex).Value.ToString.Trim <> "") Then
                    Dim cell As DataGridViewCheckBoxCell = TryCast(DG.CurrentRow.Cells(11), DataGridViewCheckBoxCell)
                    cell.Value = Not cell.Value
                End If
            End If
        Catch ex As Exception
            MessageBox.Show("Esta columna no puede ser ordenada", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End Try
    End Sub
    ...
    Private Sub SeleccionarTodos(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkTodos.CheckedChanged
        ...
        I = 0 : CONT = DG.RowCount - 1
        While I <= CONT
            If (DG.Item(4, I).Value.ToString.Trim = "") Then
                I += 1
            ElseIf ((DG.Item(0, I).Value.ToString.Trim = "03" Or DG.Item(0, I).Value.ToString.Trim = "12") AndAlso CDbl(DG.Item(10, I).Value) > MontoPle And (DG.Item(4, I).Value.ToString.Trim = "" Or DG.Item(5, I).Value.ToString.Trim = "" Or DG.Item(6, I).Value.ToString.Trim = "")) Then
                I += 1
            Else
                DG.Item(11, I).Value = chkTodos.Checked
                I += 1
            End If
