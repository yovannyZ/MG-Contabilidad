...
Public Class CONSULTA_PCTAS_PAGAR
    Public COD_AUX00, COD_COMP00, NRO_COMP00, NRO_COMP01 As String
    Public COD_PER00, COD_CTA00 As String
    Public COD_CPTO00, MON00 As String
    Public COD_SUC As String
    Public FEC1, FEC2 As DateTime

    Private Sub Cancel_Button_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Cancel_Button.Click
        DialogResult = Windows.Forms.DialogResult.Cancel
        Close()
    End Sub
    Sub CARGAR_PCTAS_PAGAR()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES", con)
            ...
            PROG_01.Parameters.Add("@COD_PER", SqlDbType.Char).Value = ""
            PROG_01.Parameters.Add("@STATUS_PER", SqlDbType.Char).Value = "1"
    ...
    Sub CARGAR_PCTAS_PAGAR_PERSONA()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
    ...
    Sub CARGAR_PCTAS_PAGAR_RANGO_DOC()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES_RANGO", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
            PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = (COD_SUC)
            PROG_01.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
            PROG_01.Parameters.Add("@FECHA1", SqlDbType.DateTime).Value = FEC1
            PROG_01.Parameters.Add("@FECHA2", SqlDbType.DateTime).Value = FEC2
            con.Open()
            PROG_01.ExecuteNonQuery()
            Dim Rs3 As SqlDataReader = PROG_01.ExecuteReader
            Do While Rs3.Read
    ...
    Sub CARGAR_PCTAS_PAGAR_PERSONA_MONEDA()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES_MONEDA", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
            PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = (COD_SUC)
            PROG_01.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
            PROG_01.Parameters.Add("@FE_MES", SqlDbType.Char).Value = MES
            ...
            PROG_01.Parameters.Add("@STATUS_PER", SqlDbType.Char).Value = "0"
            PROG_01.Parameters.Add("@COD_CPTO", SqlDbType.Char).Value = ""
            'PROG_01.Parameters.Add("@COD_MONEDA", SqlDbType.Char).Value = MON00
            con.Open()
            PROG_01.ExecuteNonQuery()
            Dim Rs3 As SqlDataReader = PROG_01.ExecuteReader
            Do While Rs3.Read
    ...
    Sub CARGAR_PCTAS_PAGAR_CUENTA()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES_CTA", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
            PROG_01.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
            PROG_01.Parameters.Add("@FE_MES", SqlDbType.Char).Value = MES
            PROG_01.Parameters.Add("@COD_PER", SqlDbType.Char).Value = COD_PER00
            PROG_01.Parameters.Add("@COD_CUENTA", SqlDbType.VarChar).Value = COD_CTA00
            con.Open()
            PROG_01.ExecuteNonQuery()
            Dim Rs3 As SqlDataReader = PROG_01.ExecuteReader
            Do While Rs3.Read
    ...
    Sub CARGAR_PCTAS_PAGAR_X_COMPROBANTE()
        DGW_CAB.Rows.Clear()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_CONSULTA_PCTAS_PAGAR_PTES2", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
            PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = (COD_SUC)
            PROG_01.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
            PROG_01.Parameters.Add("@FE_MES", SqlDbType.Char).Value = MES
            PROG_01.Parameters.Add("@COD_AUX", SqlDbType.Char).Value = COD_AUX00
            PROG_01.Parameters.Add("@COD_COMP", SqlDbType.Char).Value = COD_COMP00
            PROG_01.Parameters.Add("@NRO_COMP1", SqlDbType.VarChar).Value = NRO_COMP00
            PROG_01.Parameters.Add("@NRO_COMP2", SqlDbType.VarChar).Value = NRO_COMP01
            con.Open()
            PROG_01.ExecuteNonQuery()
            Dim Rs3 As SqlDataReader = PROG_01.ExecuteReader
            Do While Rs3.Read
    ...
    Private Sub CH_TODOS_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles CH_TODOS.CheckedChanged
        If CH_TODOS.Checked Then
            CARGAR_PCTAS_PAGAR()
        Else
            CARGAR_PCTAS_PAGAR_PERSONA()
        End If
    End Sub
    ...
    Private Sub OK_Button_Click(ByVal sender As Object, ByVal e As EventArgs) Handles OK_Button.Click
        ...
        Do While (I <= CONT)
            If (DGW_CAB.Item(11, I).Value.ToString = "True") Then
                ...
                End If
                LBL.Text = "SI"
                DGW.Rows.Add(cod_per, cod_doc, nro_doc)
            End If
            I += 1
        Loop
    End Sub
    ...
    Private Sub CheckBox1_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CheckBox1.CheckedChanged
        ...
        cont = DGW_CAB.Rows.Count - 1
        If CheckBox1.Checked = True Then
            For i = 0 To cont
                DGW_CAB.Item(9, i).Value = True
            Next
        Else
            For i = 0 To cont
                DGW_CAB.Item(9, i).Value = False
            Next
        End If
    End Sub
End Class