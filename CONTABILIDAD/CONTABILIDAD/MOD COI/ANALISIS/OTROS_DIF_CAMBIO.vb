...
Public Class OTROS_DIF_CAMBIO
    Dim FECHA_COMP0000 As DateTime
    Dim COD_AUX As String
    Dim COD_AUX2 As String
    Dim COD_COMP As String
    Dim COD_COMP2 As String
    Dim COD_CUENTA As String
    Dim COD_SUCURSAL As String
    Dim cuenta_haber As String
    Dim DT_DET As New DataTable
    Dim obj As New Class1
    Dim TC_ACT As Decimal
    Dim COD_SUCURSAL2, TOTAL0, DH0, CTA0, TOTAL0_PTE, DH0_PTE, CTA0_PTE As String
    Dim cuenta_debe, CUENTA_ENLACE, CUENTA_DESTINO As String
    Sub ASIENTO_TOTAL()
        CUENTA_ENLACE = ""
        CUENTA_DESTINO = ""
    ...
    Private Sub btn_aceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar.Click
        ...
        If (obj.ValidarCierreCuentas("OC", AÑO, obj.COD_MES(cbo_mes.Text)) = True) Then
    ...
    Private Sub btn_aceptar2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar2.Click
        ...
        'Validamos si el periodo para este tipo de  cuenta se encuentra bloqueado
        If (obj.ValidarCierreCuentas("OC", AÑO, obj.COD_MES(cbo_mes2.Text)) = True) Then
            MessageBox.Show("El periodo se encuentra bloqueado,no se pueden realizar operaciones", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
        '------------------------------------------------
    ...
    Function CREAR_ASIENTO() As String
        ...
        '---------------------------------------------------
        Try
            Dim CMD As New SqlCommand("INSERTAR_I_CON_DIF_CAMBIO_OTROS", con)
    ...
    Function CREAR_ASIENTO_PTE() As String
        ...
        Do While (I <= CONT)
            If (dgw2.Item(2, I).Value = True) Then
                ...
                If (Decimal.Compare(IMP_S, Decimal.Zero) < 0) Then
                    ...
                    IMP_S = Decimal.Multiply(IMP_S, Decimal.MinusOne)
                End If
                DT_DET.Rows.Add(AÑO, obj.COD_MES(cbo_mes2.Text), COD_AUX2, COD_COMP2, txt_nro_comp2.Text, "00", String.Format("{0}-{1}", AÑO, MES), "", "", (DT_DET.Rows.Count + 1).ToString("0000"), FECHA_COMP0000, "", "", Now.Date, "AJUSTE DE DIFERENCIA DE CAMBIO", dgw2.Item(0, I).Value.ToString, IMP_S, 0, COD_D_H0, "A", TC_ACT, "", "", "", "", Now.Date, "0", "", "", "0", "0", "0", "0", "", "", Now.Date, 0, "", "0", Now.Date, 0, NOMBRE_PC, "1")
            End If
            I += 1
        Loop
        'CUENTA DEL TOTAL DEL COMPROBANTE
        ...
        End Try
        Try
            Dim CMD As New SqlCommand("INSERTAR_I_CON_DIF_CAMBIO_OTROS_PTE", con)
            CMD.CommandType = (CommandType.StoredProcedure)
    ...
    Sub SALDOS_CUENTAS()
        ...
        Do While (I <= CONT)
            If (dgw1.Item(2, I).Value = True) Then
                ...
                Try
                    Dim cmd As New SqlCommand("DIF_CAMBIO_CONC_OTROS", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
    ...
    Sub SALDOS_CUENTAS_PTE()
        ...
        Do While (I <= CONT)
            If (dgw2.Item(2, I).Value = True) Then
                ...
                Dim IMP As New Decimal
                Try
                    Dim cmd As New SqlCommand("DIF_CAMBIO_PTE_OTROS", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
                    cmd.Parameters.Add("@COD_CUENTA", SqlDbType.Char).Value = COD_CUENTA0
                    cmd.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
    ...
    Function VERIFICAR_MSALDO_CUENTA() As Boolean
        ...
        Do While (I <= CONT)
            If (dgw2.Item(2, I).Value = True) Then
                ...
                Dim IMP As New Decimal
                Try
                    Dim cmd As New SqlCommand("VERIFICAR_SALDOS_MAESTROS_OTROS", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
                    cmd.Parameters.Add("@COD_CUENTA", SqlDbType.Char).Value = COD_CUENTA0
                    cmd.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
                    cmd.Parameters.Add("@FE_MES", SqlDbType.Char).Value = obj.COD_MES(cbo_mes2.Text)
        ...
        Return True
    End Function
    Private Sub OTROS_DIF_CAMBIO_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Sub CARGAR_CTAS()
        ...
        End Try
    End Sub
    Private Sub OTROS_DIF_CAMBIO_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
