...
Public Class CXC_DES_CONC_MANUAL
    ...
    Sub DESCONCILIAR(ByVal cod_aux1 As Object, ByVal cod_com1 As Object, ByVal nro_com1 As Object, ByVal cod_doc1 As Object, ByVal nro_doc1 As Object, ByVal cod_per1 As Object, ByVal nro_doc_per1 As Object, ByVal ITEM0 As Object)
        Try
            Dim cmd As New SqlCommand("DESCONCILIAR_DOC_A_TCXC", con)
    ...
    Sub DGW_CUENTAS00()
        Try
            ...
            dgw_cuenta.Columns(2).Visible = False
            dgw_cuenta.Columns(3).Visible = False
            dgw_cuenta.Columns(4).Visible = False
            dgw_cuenta.Columns(0).Width = 70
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    End Sub
    ...
    Sub MOSTRAR_ATCXP_CONC()
        Try
            Dim cmd As New SqlCommand("MOSTRAR_A_TCXC_CONC", con)
            cmd.CommandType = (CommandType.StoredProcedure)
            cmd.Parameters.Add("@COD_cuenta ", SqlDbType.VarChar).Value = txt_cod_cuenta.Text
            cmd.Parameters.Add("@fe_año", SqlDbType.VarChar).Value = AÑO
            cmd.Parameters.Add("@fe_mes", SqlDbType.VarChar).Value = MES00
    ...
    Private Sub txt_cod_cuenta_LostFocus(ByVal sender As Object, ByVal e As EventArgs) Handles txt_cod_cuenta.LostFocus
        If (Strings.Trim(txt_cod_cuenta.Text) <> "") Then
            If (dgw_cuenta.RowCount = 0) Then
                MessageBox.Show("No existen cuentas de tipo C", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Else
    ...
    Private Sub txt_desc_cuenta_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txt_desc_cuenta.KeyDown
        ...
        End If
    End Sub
    Private Sub CXC_DES_CONC_MANUAL_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CXC_DES_CONC_MANUAL_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
        CBO_MONEDA00()
        DGW_CUENTAS00()
    End Sub
