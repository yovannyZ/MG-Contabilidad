...
Public Class CXC_DIF_CAMBIO
    Dim cuenta_haber, COD_AUX, COD_AUX2, COD_COMP, COD_COMP2, COD_CUENTA, COD_SUCURSAL As String
    Dim cuenta_debe, CUENTA_ENLACE, CUENTA_DESTINO, COD_SUCURSAL2, TOTAL0, DH0, CTA0, TOTAL0_PTE, DH0_PTE, CTA0_PTE As String
    ...
    Private Sub btn_aceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar.Click
        ...
        TC_ACT = obj.SACAR_CAMBIO_MENSUAL(AÑO, obj.COD_MES(cbo_mes.Text), "D", "C")
    ...
    Private Sub btn_aceptar2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar2.Click
        ...
        If (VERIFICAR_MSALDO_CUENTA()) Then
            ...
            'TC_ACT = (obj.SACAR_CAMBIO(FECHA_GRAL.Year, FECHA_GRAL.ToString("MM"), FECHA_GRAL.ToString("dd"), "D"))
    ...
    Function CREAR_ASIENTO() As String
        ...
        Do While (I <= CONT)
            If dgw1.Item(2, I).Value = True Then
                ...
                End If
                'Dim VB$t_array$S0 As Object() = New Object() { AÑO, MES, COD_AUX, COD_COMP, txt_nro_comp.Text, "", "", "", "", (I + 1).ToString("0000"), Now.Date, "", "", Now.Date, "AJUSTE DE DIFERENCIA DE CAMBIO", dgw1.Item(0, I).Value.ToString, IMP_S, 0, COD_D_H0, "A", 1, "", "", "", "", Now.Date, "0", "", "", "0", "0", "0", "0", "", "", Now.Date, 0, "", "0", Now.Date, 0, NOMBRE_PC, "0" }
                DT_DET.Rows.Add(AÑO, obj.COD_MES(cbo_mes.Text), COD_AUX, COD_COMP, txt_nro_comp.Text, "00", String.Format("{0}-{1}", AÑO, MES), "", "", (Me.DT_DET.Rows.Count + 1).ToString("0000"), FECHA_COMP0000, "", "", FECHA_GRAL.Date, "AJUSTE DE DIFERENCIA DE CAMBIO", dgw1.Item(0, I).Value.ToString, IMP_S, 0, COD_D_H0, "A", TC_ACT, "", "", "", "", FECHA_GRAL.Date, "0", "", "", "0", "0", "0", "0", "", "", FECHA_GRAL.Date, 0, "", "0", Now.Date, 0, NOMBRE_PC, "1")
            End If
            I += 1
        Loop
        ...
        End Try
        Try
            Dim CMD As New SqlCommand("INSERTAR_I_CON_DIF_CAMBIO_CXC", con)
    ...
    Function CREAR_ASIENTO_PTE() As String
        ...
        Do While (I <= CONT)
            If dgw2.Item(2, I).Value = True Then
                ...
                If (Decimal.Compare(IMP_S, Decimal.Zero) < 0) Then
                    ...
                    IMP_S = Decimal.Multiply(IMP_S, Decimal.MinusOne)
                End If
                'Dim VB$t_array$S0 As Object() = New Object() { AÑO, MES, COD_AUX2, COD_COMP2, txt_nro_comp2.Text, "", "", "", "", (I + 1).ToString("0000"), Now.Date, "", "", Now.Date, "AJUSTE DE DIFERENCIA DE CAMBIO", dgw2.Item(0, I).Value.ToString, IMP_S, 0, COD_D_H0, "A", 1, "", "", "", "", Now.Date, "0", "", "", "0", "0", "0", "0", "", "", Now.Date, 0, "", "0", Now.Date, 0, NOMBRE_PC, "0" }
        ...
        End Try
        Try
            Dim CMD As New SqlCommand("INSERTAR_I_CON_DIF_CAMBIO_CXC_PTE", con)
            CMD.CommandType = (CommandType.StoredProcedure)
    ...
    Sub SALDOS_CUENTAS()
        ...
        Do While (I <= CONT)
            If dgw1.Item(2, I).Value = True Then
                ...
                Try
                    Dim cmd As New SqlCommand("DIF_CAMBIO_CONC_CXC", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
    ...
    Sub SALDOS_CUENTAS_PTE()
        ...
        Do While (I <= CONT)
            If dgw2.Item(2, I).Value = True Then
                ...
                Dim IMP As New Decimal
                Try
                    Dim cmd As New SqlCommand("DIF_CAMBIO_PTE_CXC", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
                    cmd.Parameters.Add("@COD_CUENTA", SqlDbType.Char).Value = (COD_CUENTA0)
                    cmd.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
    ...
    Function VERIFICAR_MSALDO_CUENTA() As Boolean
        ...
        Do While (I <= CONT)
            If dgw2.Item(2, I).Value = True Then
                ...
                Dim IMP As New Decimal
                Try
                    Dim cmd As New SqlCommand("VERIFICAR_SALDOS_MAESTROS_CXC", con)
                    cmd.CommandType = (CommandType.StoredProcedure)
                    cmd.Parameters.Add("@COD_CUENTA", SqlDbType.Char).Value = (COD_CUENTA0)
                    cmd.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = AÑO
                    cmd.Parameters.Add("@FE_MES", SqlDbType.Char).Value = obj.COD_MES(cbo_mes2.Text)
        ...
        Return True
    End Function
    Private Sub CXC_DIF_CAMBIO_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CXC_DIF_CAMBIO_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
