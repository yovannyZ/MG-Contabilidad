...
Public Class OTROS_TRANSF_AÑO
    ...
    Private Sub btn_aceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar.Click
        If MES <> "12" Then
            ...
            Exit Sub
        End If
        Dim nrorow As Integer = (dgw1.RowCount - 1)
        Dim c As Integer = 0
        'Do While (c <= nrorow)
        '    If dgw1.Item(2, c).Value = True And dgw1.Item(3, c).Value = True Then
        '        MessageBox.Show("No puede activar a la vez con y sin diferencia de cambio")
        '        Exit Sub
        '    End If
        '    c += 1
        'Loop
        ...
        Do While (j <= k)
            ...
            If dgw1.Item(2, j).Value = True Then
                ...
                Try
                    Dim cmd As New SqlCommand("TRANSFERIR_CTAS_OTROS_AÑO", con)
    ...
    Function VERIFICAR_TRANSF_AÑO() As Boolean
        ...
        For I = 0 To CONT
            Dim ESTADO = "FALLO"
            Try
                Dim cmd As New SqlCommand("VERIFICAR_TRANSF_AÑO_OTROS", con)
                cmd.CommandType = (CommandType.StoredProcedure)
    ...
    Sub DESCONCILIAR_CUENTA(ByVal cod_cta As String)
        Try
            Dim cmd As New SqlCommand("DESTRANSFERIR_CTAS_OTROS_AÑO", con)
            cmd.CommandType = (CommandType.StoredProcedure)
