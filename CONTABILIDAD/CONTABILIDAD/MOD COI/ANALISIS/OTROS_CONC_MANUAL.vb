...
Public Class OTROS_CONC_MANUAL
    ...
    Sub CONCILIAR(ByVal cod_aux1 As Object, ByVal cod_com1 As Object, ByVal nro_com1 As Object, ByVal cod_doc1 As Object, ByVal nro_doc1 As Object, ByVal cod_per1 As Object, ByVal nro_doc_per1 As Object, ByVal ITEM0 As Object)
        Try
            Dim cmd As New SqlCommand("CONCILIAR_DOC_A_TOTROS", con)
    ...
    Sub DESTRANSFERIR(ByVal cod_aux1 As Object, ByVal cod_com1 As Object, ByVal nro_com1 As Object, ByVal cod_doc1 As Object, ByVal nro_doc1 As Object, ByVal cod_per1 As Object, ByVal nro_doc_per1 As Object)
        Try
            Dim cmd As New SqlCommand("DESTRANSFERIR_DOC_A_TOTROS", con)
            cmd.CommandType = (CommandType.StoredProcedure)
            cmd.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = cod_aux1
            cmd.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = cod_com1
            cmd.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = nro_com1
    ...
    Sub HALLAR_MONTO()
        ...
        Dim I As Integer = 0
        Dim CONT As Integer = (DGW_IE.RowCount - 1)
        If ch_moneda.Checked = True Then
            ...
            Loop
        Else
            'Dim VB$t_i4$L1 As Integer = CONT
            I = 0
            Do While (I <= CONT)
    ...
    Sub MOSTRAR_ATCXP()
        Try
            Dim cmd As New SqlCommand("MOSTRAR_A_TOTROS_PTE_CONC", con)
            cmd.CommandType = (CommandType.StoredProcedure)
            cmd.Parameters.Add("@COD_cuenta ", SqlDbType.VarChar).Value = txt_cod_cuenta.Text
            ...
            DGW_IE.Columns.Item(1).Width = 85
            DGW_IE.Columns.Item(2).Width = 50
            DGW_IE.Columns.Item(3).Width = 180
            DGW_IE.Columns.Item(4).Width = 75
            DGW_IE.Columns.Item(5).Width = 30
            DGW_IE.Columns.Item(6).Width = 95
            DGW_IE.Columns.Item(7).Width = 95
            DGW_IE.Columns.Item(6).DefaultCellStyle.Alignment = 1024
            DGW_IE.Columns.Item(7).DefaultCellStyle.Alignment = 1024
            DGW_IE.Columns.Item(8).Width = 20
            DGW_IE.Columns.Item(9).Width = 35
            DGW_IE.Columns.Item(10).Width = 45
            DGW_IE.Columns.Item(11).Width = 75
            DGW_IE.Columns.Item(12).Width = 30
            DGW_IE.Columns.Item(13).Width = 35
            DGW_IE.Columns.Item(14).Width = 45
            DGW_IE.Columns.Item(15).Width = 70
            DGW_IE.Columns(16).Width = 35
            DGW_IE.Columns(17).Visible = False
            DGW_IE.Columns(18).Width = 180
        Catch ex As Exception
            MsgBox(ex.Message)
            MessageBox.Show("Vuelva a Intentarlo", "Ocurrió un error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End Try
    ...
    Private Sub txt_cod_cuenta_LostFocus(ByVal sender As Object, ByVal e As EventArgs) Handles txt_cod_cuenta.LostFocus
        If (Strings.Trim(txt_cod_cuenta.Text) <> "") Then
            If (dgw_cuenta.RowCount = 0) Then
                MessageBox.Show("No existen cuentas de O", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Else
    ...
    Private Sub txt_desc_cuenta_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txt_desc_cuenta.KeyDown
        If ((e.KeyData = Keys.Return) AndAlso (Strings.Trim(txt_desc_cuenta.Text) <> "")) Then
            If (dgw_cuenta.RowCount = 0) Then
                MessageBox.Show("No existen cuentas de tipo O", "Mensaje")
            Else
                dgw_cuenta.Sort(dgw_cuenta.Columns.Item(0), System.ComponentModel.ListSortDirection.Ascending)
                'Dim VB$t_i4$L0 As Integer = (dgw_cuenta.RowCount - 1)
                Dim i As Integer = 0
        ...
        End If
    End Sub
    Private Sub OTROS_CONC_MANUAL_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub OTROS_CONC_MANUAL_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
        DGW_CUENTAS00()
        CBO_MONEDA00()
