...
Public Class BAN_INGRESO_ANA
    Dim BOTON, COD_AUX, SOL, DOLAR, COD_REF, MES00, COD_COMP, COD_DOC, COD_MONEDA As String
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If (Decimal.Parse((CInt(MessageBox.Show("Desea borrar el Comprobante", " Borrar Comprobante?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
            ...
            Try
                Dim comand1 As New SqlCommand("ELIMINAR_A_TBAN", con)
    ...
    Private Sub btn_eliminar2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar2.Click
        ...
        If (Decimal.Parse((CInt(MessageBox.Show("Esta seguro de eliminar el detalles", "Advertencia", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
    ...
    Private Sub btn_grabar_com_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_grabar_com.Click
        ...
        If (dgw_mov1.RowCount = 0) Then MessageBox.Show("Comprobante sin Detalles", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : btn_nuevo_det.Focus() : Exit Sub
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        '---------------------------------------------------------------------
        If (Panel_CXP.Visible Or (Strings.Trim(txt_cod_per0.Text) = "")) Then MessageBox.Show("Debe elegir la Persona", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_cod_per0.Focus() : Exit Sub
        If ((Strings.Trim(txt_cod_cta0.Text) = "") Or Panel_cta.Visible) Then MessageBox.Show("Debe elegir la Cuenta", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_cod_cta0.Focus() : Exit Sub
        If obj.VERIFICAR_CIERRE_CUENTAS(txt_cod_cta0.Text, AÑO, MES00) = False Then Exit Sub
        If (cbo_tipo_doc0.SelectedIndex = -1) Then MessageBox.Show("Debe elegir el Documento", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : cbo_tipo_doc0.Focus() : Exit Sub
        If ((Strings.Trim(txt_nro_doc0.Text) = "") Or Panel_cta.Visible) Then MessageBox.Show("Debe ingresar el Nº de Documento", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_nro_doc0.Focus() : Exit Sub
        ...
        If ((txt_cambio0.Text.Trim = "") Or (txt_cambio0.Text.Trim = "0.0000")) Then MessageBox.Show("Debe ingresar el  Tipo de Cambio", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_cambio0.Focus() : Exit Sub
        If ((txt_imp_soles0.Text.Trim = "") Or (txt_imp_soles0.Text.Trim = "0.0000")) Then MessageBox.Show("Debe ingresar el  Importe", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : txt_imp_soles0.Focus() : Exit Sub
        If (Decimal.Parse(obj.VALIDAR_FECHA_DOC(dtp_doc0.Value)) = 0) Then
            dtp_doc0.Focus()
        End If
        'Else
        ...
        Else
            ...
            dgw_mov1.Rows.Add(txt_cod_cta0.Text, txt_glosa0.Text, SOL, DOLAR, cbo_d_h.Text, COD_MONEDA, txt_cambio0.Text, COD_DOC, txt_nro_doc0.Text, dtp_doc0.Value, txt_cod_per0.Text, txt_desc_per0.Text, txt_nro_doc_per0.Text, COD_REF, txt_nro_ref0.Text, txt_imp_soles0.Text)
            HALLAR_TOTAL()
            LIMPIAR_DOCUMENTO()
            txt_cod_per0.Focus()
        End If
    ...
    Sub CARGAR_COMPROBANTE()
        ...
        dgw_mov1.Rows.Clear()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_T_A_TBAN", con)
    ...
    Sub CARGAR_DETALLE1()
        ...
        dtp_doc0.Value = DateTime.Parse(dgw_mov1.Item(9, dgw_mov1.CurrentRow.Index).Value)
    ...
    Sub CREAR_DATASET()
        ...
        DT_DET.Columns.Add("FECHAconciliado")
    ...
    Private Sub CXP_INGRESO_ANA_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

    Private Sub CXP_INGRESO_ANA_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        KeyPreview = True
        'DATAGRID()
    ...
    Sub DATAGRID()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_A_TBAN", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@fe_año", SqlDbType.VarChar).Value = AÑO
            pro_02.Parameters.Add("@fe_mes", SqlDbType.VarChar).Value = MES00
    ...
    Sub DGW_CUENTAS()
        Try
            dgw_cuenta.DataSource = OBJ.MOSTRAR_CUENTAS_STATUS_TIPO(AÑO, "B")
    ...
    Function INSERTAR_TODO() As String
        ...
        Try
            con.Open()
            sqlbc.DestinationTableName = "dbo.ANALISIS_TBAN2"
        ...
        End Try
        Try
            Dim comand1 As New SqlCommand("INSERTAR_A_TBAN", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
    ...
    Sub LIMPIAR_DOCUMENTO()
        ...
        Dim DESC_MON As String = cbo_moneda1.Text
        Dim TC As String = txt_cambio0.Text
        Try
    ...
    Function MODIFICAR_TODO() As String
        ...
        End Try
        Try
            Dim comand1 As New SqlCommand("MODIFICAR_A_TBAN", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
