...
Public Class OTROS_TRANSF_ANA
    ...
    Private Sub btn_aceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_aceptar.Click
        ...
        Do While (j <= k)
            ...
            If (dgw1.Item(2, j).Value = True) Then
                ...
                Try
                    Dim cmd As New SqlCommand("TRANSFERIR_CTAS_OTROS", con)
    ...
    Sub DESCONCILIAR_CUENTA(ByVal cod_cta As String)
        Try
            Dim cmd As New SqlCommand("DESTRANSFERIR_CTAS_OTROS", con)
            cmd.CommandType = (CommandType.StoredProcedure)
    ...
    Sub DGW_CUENTAS00()
        Try
            ...
            dgw2.Columns(0).ReadOnly = True
            dgw2.Columns(1).ReadOnly = True
        Catch ex As Exception
            con.Close()
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub OTROS_TRANSF_ANA_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{tab}")
        End If
    End Sub
    Private Sub OTROS_TRANSF_ANA_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        DGW_CUENTAS00()
