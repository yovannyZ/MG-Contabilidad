...
Public Class BAN_DES_CONC_MANUAL
    ...
    Private Sub btn_grabar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_grabar.Click
        ...
        Else
            ...
            m = 0
            Do While (m <= l)
                If DGW_IE.Item(9, m).Value.ToString = "True" Then
                    ...
                    DESCONCILIAR(cod_aux0, cod_com0, nro_com0, cod_doc0, nro_doc0, cod_per0, nro_doc_per0)
                End If
                m += 1
            Loop
    ...
    End Sub
    Private Sub Desconciliacion_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub Desconciliacion_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        KeyPreview = True
        CBO_MONEDA00()
        DGW_CUENTAS00()
    End Sub
    Sub DESCONCILIAR(ByVal cod_aux1 As Object, ByVal cod_com1 As Object, ByVal nro_com1 As Object, ByVal cod_doc1 As Object, ByVal nro_doc1 As Object, ByVal cod_per1 As Object, ByVal nro_doc_per1 As Object)
        Try
            Dim cmd As New SqlCommand("DESCONCILIAR_DOC_A_TBAN", con)
 ...
 Sub HALLAR_TOTAL()
    ...
    Sub MOSTRAR_ATCXP_CONC()
        Try
            Dim cmd As New SqlCommand("MOSTRAR_A_TBAN_CONC", con)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.Add("@COD_cuenta ", SqlDbType.VarChar).Value = txt_cod_cuenta.Text
            cmd.Parameters.Add("@fe_año", SqlDbType.VarChar).Value = AÑO
            cmd.Parameters.Add("@fe_mes", SqlDbType.VarChar).Value = MES00
            ...
            DGW_IE.Columns.Item(1).Width = &H55
            DGW_IE.Columns.Item(2).Width = 50
            DGW_IE.Columns.Item(3).Width = 180
            DGW_IE.Columns.Item(4).Width = &H4B
            DGW_IE.Columns.Item(5).Width = 30
            DGW_IE.Columns.Item(6).Width = &H5F
            DGW_IE.Columns.Item(7).Width = &H5F
            DGW_IE.Columns.Item(6).DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomRight
            DGW_IE.Columns.Item(7).DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomRight
            DGW_IE.Columns.Item(8).Width = 20
            DGW_IE.Columns.Item(9).Width = 35
            DGW_IE.Columns.Item(10).Width = &H4B
            DGW_IE.Columns.Item(11).Width = 30
            DGW_IE.Columns.Item(12).Width = 35
            DGW_IE.Columns.Item(13).Width = &H2D
            DGW_IE.Columns.Item(14).Width = 70
        Catch ex As Exception
            MsgBox(ex.Message)
            MessageBox.Show("Vuelva a Intentarlo", "Ocurrió un error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End Try
    ...
    Private Sub txt_cod_cuenta_LostFocus(ByVal sender As Object, ByVal e As EventArgs) Handles txt_cod_cuenta.LostFocus
        If (Strings.Trim(txt_cod_cuenta.Text) <> "") Then
            If (dgw_cuenta.RowCount = 0) Then
                MessageBox.Show("No existen cuentas de tipo B", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Else
