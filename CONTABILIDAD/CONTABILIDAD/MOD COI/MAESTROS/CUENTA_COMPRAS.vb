...
Public Class CUENTA_COMPRAS
    Dim boton As String
    Dim COD_ORDEN, COD_ORDEN2 As String
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If MessageBox.Show(("ELIMINAR " & COD_CLASE & " " & dgw1.Item(1, dgw1.CurrentRow.Index).Value.ToString), "ADVERTENCIA", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = 6 Then
            Try
                ...
                CMD.Parameters.Add("@TIPO_MOV ", SqlDbType.Char).Value = ("C")
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        ElseIf (cbo_orden.SelectedIndex = -1) Then
            ...
            cbo_orden.Focus()
        ElseIf (cbo_orden2.SelectedIndex = -1) And cbo_orden2.Visible = True Then
            MessageBox.Show("Debe seleccionar el Nro de Orden (Sin Igv).", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            cbo_orden2.Focus()
        Else
            ...
            End If
            COD_ORDEN2 = ""
            If cbo_orden2.SelectedIndex <> -1 Then COD_ORDEN2 = OBJ.COD_NRO_ORDEN(cbo_orden2.Text, "C")
            COD_ORDEN = OBJ.COD_NRO_ORDEN(cbo_orden.Text, "C")
            If (CONTAR_REG() > 0) Then
                MessageBox.Show("El codigo de Cuenta Compra ya existe", "YA EXISTE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                txt_cod_cuenta.Focus()
            Else
                Try
                    ...
                    CMD.Parameters.Add("@TIPO_MOV", SqlDbType.VarChar).Value = "C"
                    con.Open()
                    CMD.ExecuteNonQuery()
                    con.Close()
                    MessageBox.Show("La Configuración de Cuenta Compras se guardo", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    LIMPIAR()
                    TabControl1.SelectedTab = TabPage1
                Catch ex As Exception
                    con.Close()
    ...
    Sub CARGAR_DATOS()
        ...
        COD_ORDEN2 = dgw1.Item(5, dgw1.CurrentRow.Index).Value.ToString
        cbo_orden2.Text = dgw1.Item(6, dgw1.CurrentRow.Index).Value.ToString
        If (TIPO_ORDEN = "T") Then
            cbo_tipo.Text = "TOTAL"
        ElseIf (TIPO_ORDEN = "I") Then
            cbo_tipo.Text = "IGV"
    ...
    Private Sub txt_des_cuenta_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txt_des_cuenta.KeyDown
        ...
        End If
    End Sub
    Private Sub CUENTA_COMPRAS_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CUENTA_COMPRAS_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
        DATAGRID()
        btn_nuevo.Select()
        CARGAR_CUENTA()
