...
Public Class CUENTA_VENTAS
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If MessageBox.Show(("ELIMINAR " & COD_CLASE & " " & dgw1.Item(1, dgw1.CurrentRow.Index).Value.ToString), "ADVERTENCIA", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = 6 Then
            Try
                ...
                CMD.Parameters.Add("@TIPO_MOV ", SqlDbType.Char).Value = ("V")
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        Else
            ...
            End If
            COD_ORDEN2 = ""
            If cbo_orden2.SelectedIndex <> -1 Then COD_ORDEN2 = OBJ.COD_NRO_ORDEN(cbo_orden2.Text, "V")
            COD_ORDEN = OBJ.COD_NRO_ORDEN(cbo_orden.Text, "V")
            If (CONTAR_REG > 0) Then
                MessageBox.Show("El codigo de Cuenta Venta ya existe", "YA EXISTE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                txt_cod_cuenta.Focus()
            Else
                Try
                    ...
                    CMD.Parameters.Add("@TIPO_MOV", SqlDbType.VarChar).Value = ("V")
                    CMD.Parameters.Add("@NRO_ORDEN2", SqlDbType.Char).Value = COD_ORDEN2
                    con.Open()
                    CMD.ExecuteNonQuery()
                    con.Close()
                    MessageBox.Show("La Configuración de Cuenta Venta se guardo", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    LIMPIAR()
                    TabControl1.SelectedTab = (TabPage1)
                Catch ex As Exception
    ...
    Sub CBO_ORDEN_COMPRAS()
        ...
        Try
            Dim PROG_01 As New SqlCommand("CBO_ORDEN_VENTAS", con)
            con.Open()
    ...
    Private Sub txt_des_cuenta_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txt_des_cuenta.KeyDown
        ...
        End If
    End Sub
    Private Sub CUENTA_VENTAS_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CUENTA_VENTAS_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
        DATAGRID()
        btn_nuevo.Select()
        CARGAR_CUENTA()
    ...
    Private Sub btn_modificar2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_modificar2.Click
        ...
        Try
            ...
            CMD.ExecuteNonQuery()
            con.Close()
            MessageBox.Show("La Configuración de Cuenta Ventas se guardo", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            TabControl1.SelectedTab = TabPage1
        Catch ex As Exception
            con.Close()
            MsgBox(ex.Message)
