...
Public Class CONSULTA_COI_CXP1
    Dim COD_CUENTA, COD_SUCURSAL, ST_CTA, STATUS_SUC As String
    ...
    Private Sub BTN_CONSULTA1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BTN_CONSULTA1.Click
        If ch_si1.Checked = True And CBO_SUCURSAL1.SelectedIndex = -1 Then MessageBox.Show("Debe elegir la Sucursal", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) : CBO_SUCURSAL1.Focus() : Exit Sub
    ...
    Sub HALLAR_TOTALES()
        ...
        For I = 0 To CONT
            ...
            End If
        Next
        If sum_soles <> "0.00" Then
            txt_soles.Text = OBJ.HACER_DECIMAL(sum_soles * -1)
        Else
            txt_soles.Text = "0.00"
        End If
        If sum_dolares <> "0.00" Then
            txt_dolares.Text = OBJ.HACER_DECIMAL(sum_dolares * -1)
        Else
            txt_dolares.Text = "0.00"
        End If
    End Sub
    ...
    Private Sub ch_cta_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ch_cta.CheckedChanged
        'txt_cod_cta0.Enabled = ch_cta.Checked
        'txt_desc_cta0.Enabled = ch_cta.Checked
    ...
    Sub DGW_CUENTAS()
        Try
            dgw_cta.DataSource = (OBJ.MOSTRAR_CUENTAS_STATUS_TIPO(AÑO, "P"))
    ...
    Sub MOSTRAR_CONSULTA()
        If TabControl1.SelectedIndex = 0 Then
            ...
            DGW_DET1.Rows.Clear()
            Try
                Dim PROG_01 As New SqlCommand("CONSULTA_CXP_PENDIENTES_COI", con)
                ...
                Do While Rs3.Read
                    If Rs3.GetValue(4) <> "0.00" Then
                        DGW_DET1.Rows.Add((Rs3.GetValue(0)), Rs3.GetValue(1), Rs3.GetValue(2), Rs3.GetValue(3), Rs3.GetValue(4), _
        ...
        ElseIf TabControl1.SelectedIndex = 1 Then
            ...
            DGW_DET2.Rows.Clear()
            Try
                Dim PROG_01 As New SqlCommand("CONSULTA_CXP_CANCELADAS_COI", con)
                PROG_01.CommandType = (CommandType.StoredProcedure)
                PROG_01.Parameters.Add("@COD_PER", SqlDbType.Char).Value = TXT_COD1.Text
                PROG_01.Parameters.Add("@STATUS_SUC", SqlDbType.Char).Value = STATUS_SUC
                PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = COD_SUCURSAL
        ...
        ElseIf TabControl1.SelectedIndex = 2 Then
            ...
            DGW_DET3.Rows.Clear()
            Try
                Dim PROG_01 As New SqlCommand("CONSULTA_CXP_KARDEX_COI", con)
                PROG_01.CommandType = (CommandType.StoredProcedure)
                PROG_01.Parameters.Add("@COD_PER", SqlDbType.Char).Value = TXT_COD1.Text
                PROG_01.Parameters.Add("@STATUS_SUC", SqlDbType.Char).Value = STATUS_SUC
                PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = COD_SUCURSAL
    ...
    Private Sub TXT_DOC_PER1_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txt_doc_per1.KeyDown
        ...
        End If
    End Sub
    Private Sub CONSULTA_COI_CXP1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CONSULTA_COI_CXP1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        KeyPreview = True
    ...
    Sub HISTORIAL()
        ...
        Try
            Dim PROG_01 As New SqlCommand("CONSULTA_CXP_KARDEX_COI2", con)
            PROG_01.CommandType = (CommandType.StoredProcedure)
            PROG_01.Parameters.Add("@COD_PER", SqlDbType.Char).Value = TXT_COD1.Text
            PROG_01.Parameters.Add("@STATUS_SUC", SqlDbType.Char).Value = STATUS_SUC
            PROG_01.Parameters.Add("@COD_SUCURSAL", SqlDbType.Char).Value = COD_SUCURSAL
