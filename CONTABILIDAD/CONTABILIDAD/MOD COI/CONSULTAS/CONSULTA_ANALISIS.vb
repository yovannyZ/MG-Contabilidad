...
Public Class CONSULTA_ANALISIS
    ...
    Private Sub Button2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button2.Click
        ...
        btn_consulta.Select()
    End Sub
    Private Sub Button3_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_cancelar.Click
        TabControl1.SelectedTab = TabPage1
        btn_consulta.Select()
    End Sub
    Private Sub Button5_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_nuevo.Click
        limpiar_kardex()
        txt_cod_per0.Focus()
    End Sub
    Sub CARGAR_CONCILIADO()
        ...
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ANALISIS_CONCILIADO", con)
        ...
        Do While (I <= CONT)
            If dgw_conciliado.Item(8, I).Value = "D" Then
                If dgw_conciliado.Item(4, I).Value = "S" Then
                    DEBES = Decimal.Parse(Decimal.Add(DEBES, dgw_conciliado.Item(6, I).Value))
                Else
                    DEBED = Decimal.Parse(Decimal.Add(DEBED, dgw_conciliado.Item(7, I).Value))
                End If
            ElseIf dgw_conciliado.Item(4, I).Value = "S" Then
                HABERS = Decimal.Parse(Decimal.Add(HABERS, dgw_conciliado.Item(6, I).Value))
            Else
                HABERD = Decimal.Parse(Decimal.Add(HABERD, dgw_conciliado.Item(7, I).Value))
            End If
            I += 1
        Loop
    ...
    Sub CARGAR_KARDEX_CONCILIADO()
        dgw_kardex.Rows.Clear()
        Dim MES6 As String = (Decimal.Parse((obj.COD_MES(cbo_mes3.Text)))).ToString("00")
        '(obj.COD_MES(cbo_mes3.Text))
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ANALISIS_KARDEX_CONCILIADO", con)
            pro_02.CommandType = CommandType.StoredProcedure
        ...
        Do While (I <= CONT)
            If dgw_kardex.Item(8, I).Value = "D" Then
                If dgw_kardex.Item(4, I).Value = "S" Then
                    DEBES = Decimal.Parse(Decimal.Add(DEBES, dgw_kardex.Item(6, I).Value))
                Else
                    DEBED = Decimal.Parse(Decimal.Add(DEBED, dgw_kardex.Item(7, I).Value))
                End If
            ElseIf dgw_kardex.Item(4, I).Value = "S" Then
                HABERS = Decimal.Parse(Decimal.Add(HABERS, dgw_kardex.Item(6, I).Value))
            Else
                HABERD = Decimal.Parse(Decimal.Add(HABERD, dgw_kardex.Item(7, I).Value))
            End If
            I += 1
        Loop
    ...
    Sub CARGAR_KARDEX_NO_CONCILIADO()
        Dim MES6 As String = (Decimal.Parse((obj.COD_MES(cbo_mes3.Text))) + 1).ToString("00")
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ANALISIS_KARDEX_NO_CONCILIADO", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = cbo_año3.Text
            pro_02.Parameters.Add("@COD_PER", SqlDbType.Char).Value = txt_cod_per0.Text
            pro_02.Parameters.Add("@FE_MES", SqlDbType.Char).Value = MES6
    ...
    '        Dim rs2 As SqlDataReader = pro_02.ExecuteReader
    '        Do While rs2.Read
    '            Dim CONT As Object = obj.COD_MES(cbo_mes.Text)
    '            If CONT = "00" Then
    ...
    Sub CARGAR_MAESTROS_CUENTAS()
        ...
        Try
            ...
            pro_02.Parameters.Add("@STATUS_ANA", SqlDbType.VarChar).Value = "P"
            con.Open()
            pro_02.ExecuteNonQuery()
            Dim rs2 As SqlDataReader = pro_02.ExecuteReader
            Do While rs2.Read
    ...
    '    txt_mov.Clear()
    '    Try
    '        Dim pro_02 As New SqlCommand("MOSTRAR_SALDO_MOV", con)
    '        pro_02.CommandType = CommandType.StoredProcedure
    '        pro_02.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = cbo_año.Text
    '        pro_02.Parameters.Add("@COD_CUENTA", SqlDbType.VarChar).Value = dgw_cuenta.Item(0, dgw_cuenta.CurrentRow.Index).Value.ToString
    ...
    Sub CARGAR_PENDIENTE_DEFECTO()
        ...
        prueba()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ANALISIS_TCXP", con)
            pro_02.CommandType = CommandType.StoredProcedure
            'pro_02.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = cbo_año.Text
            'pro_02.Parameters.Add("@FE_MES", SqlDbType.Char).Value = MES0
            'pro_02.Parameters.Add("@COD_CUENTA", SqlDbType.VarChar).Value = dgw_cuenta.Item(0, dgw_cuenta.CurrentRow.Index).Value.ToString
    ...
    Sub CARGAR_PENDIENTE_DEFECTO_CONCILIADO()
        Dim MES1 As String = (Decimal.Parse((obj.COD_MES(cbo_mes.Text))) + 1).ToString("00")
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_ANALISIS_TCXP2", con)
            pro_02.CommandType = CommandType.StoredProcedure
            pro_02.Parameters.Add("@FE_AÑO", SqlDbType.Char).Value = cbo_año.Text
    ...
    End Sub
    Private Sub CONSULTA_ANALISIS_FormClosing(ByVal sender As Object, ByVal e As FormClosingEventArgs) Handles Me.FormClosing
        main(&H54) = 0
    End Sub
    Private Sub CONSULTA_ANALISIS_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Return) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    Private Sub CONSULTA_ANALISIS_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        KeyPreview = True
        CARGAR_AÑO()
        cbo_año.Text = AÑO
        cbo_año2.Text = AÑO
    ...
    Sub DGW_PERSONAS()
        Try
            ...
            dgw_per1.DataSource = obj.MOSTRAR_PERSONAS_PAGAR
