...
Public Class RV
    ...
    Sub CARGAR_TITULOS()
        ...
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_TITULOS_RV", con)
    ...
    Sub DATAGRID()
        Try
            Dim pro_02 As New SqlCommand("MOSTRAR_CONSULTA_RV", con)
    ...
    Sub CARGAR_DATOS_CABECERA()
        ...
        cbo_tipo_ref.Text = OBJ.DESC_DOC(dgw1.Item(11, dgw1.CurrentRow.Index).Value)
        txt_nro_ref.Text = dgw1.Item(12, dgw1.CurrentRow.Index).Value
        dtp_ref.Value = dgw1.Item(13, dgw1.CurrentRow.Index).Value
    End Sub
    Sub limpiar()
    ...
    Sub CARGAR_DETALLES_DGW()
        Try
            Dim PROG_01 As New SqlCommand("MOSTRAR_DETALLES_CONSULTA_RV", con)
            PROG_01.CommandType = CommandType.StoredProcedure
    ...
    Sub ACTUALIZAR_IMPORTES()
        ...
        Next
        Try
            Dim CMD As New SqlCommand("MODIFICAR_IMPORTES_RV", con)
            ...
            CMD.Parameters.Add("@COD_REF", SqlDbType.VarChar).Value = IIf(String.IsNullOrEmpty(OBJ.COD_DOC(cbo_tipo_ref.Text)), "", OBJ.COD_DOC(cbo_tipo_ref.Text))
            ...
            CMD.ExecuteNonQuery()
            con.Close()
            MessageBox.Show("Los importes del Registro de Ventas se actualizaron", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            TabControl1.SelectedTab = TabPage1
        Catch ex As Exception
            con.Close()
            MsgBox(ex.Message)
