...
Public Class CANCELACION_CXP2
    Dim BOTON, VAR_PRO, STATUS_PER, STATUS_P, COD_COMP2, COD_AUX2, STATUS_DOC, STATUS_CC, STATUS_ANA, ITEM_PAGO, cod_ref, COD_PROYECTO, COD_MP, COD_MONEDA, COD_MON_DOC, COD_AUX, COD_CC, COD_COMP, COD_CONTROL, COD_D_H, COD_D_H_PAGO, COD_DOC As String
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        If (Decimal.Parse((CInt(MessageBox.Show("Desea borrar el Comprobante", " Borrar Comprobante?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
            ...
            Try
                Dim comand1 As New SqlCommand("ELIMINAR_I_CON_CXP_CANCELACION", con)
        ...
        Do While (k1 <= k2)
            If dgw_mov.Item(25, k1).Value = ITEM_PAGO And dgw_mov.Item(26, k1).Value.ToString = "0" Then
                ...
                Else
                    ...
                    End If
                End If
                dgw_mov1.Rows.Add((dgw_mov1.RowCount + 1), (dgw_mov.Item(1, k1).Value), (dgw_mov.Item(2, k1).Value), (dgw_mov.Item(4, k1).Value), (dgw_mov.Item(5, k1).Value), (dgw_mov.Item(6, k1).Value), PAGO, (dgw_mov.Item(8, k1).Value), (dgw_mov.Item(9, k1).Value), (dgw_mov.Item(7, k1).Value), (dgw_mov.Item(10, k1).Value), (dgw_mov.Item(3, k1).Value), (dgw_mov.Item(11, k1).Value), (dgw_mov.Item(12, k1).Value), (dgw_mov.Item(17, k1).Value), (dgw_mov.Item(13, k1).Value), (dgw_mov.Item(14, k1).Value), (dgw_mov.Item(15, k1).Value), (dgw_mov.Item(18, k1).Value), (dgw_mov.Item(19, k1).Value), (dgw_mov.Item(20, k1).Value), (dgw_mov.Item(16, k1).Value), (dgw_mov.Item(21, k1).Value), (dgw_mov.Item(22, k1).Value), (dgw_mov.Item(23, k1).Value))
            End If
            k1 += 1
        Loop
    End Sub
    ...
    Sub HALLAR_TOTAL()
        ...
        Do While (I <= CONT)
            If (dgw_mov1.Item(9, I).Value.ToString = "D") Then
                ...
                '        D_DOLARES = Math.Round(Decimal.Add(D_DOLARES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString) / Decimal.Parse(txt_cambio_pago.Text)))))), 2)

                '    Case "D"
                '        D_DOLARES = Math.Round(Decimal.Add(D_DOLARES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString)))))), 2)
                '        D_SOLES = Math.Round(Decimal.Add(D_SOLES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString) * Decimal.Parse(txt_cambio_pago.Text)))))), 2)
                '    Case "A"
                '        D_SOLES = Math.Round(Decimal.Add(D_SOLES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString)))))), 2)
            ...
            Else
                ...
                '    Case "S"
                '        H_SOLES = Math.Round(Decimal.Add(H_SOLES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString)))))), 2)
                '        H_DOLARES = Math.Round(Decimal.Add(H_DOLARES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString) / Decimal.Parse(txt_cambio_pago.Text)))))), 2)

                '    Case "D"
                '        H_DOLARES = Math.Round(Decimal.Add(H_DOLARES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString)))))), 2)
                '        H_SOLES = Math.Round(Decimal.Add(H_SOLES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString) * Decimal.Parse(txt_cambio_pago.Text)))))), 2)
                '    Case "A"
                '        H_SOLES = Math.Round(Decimal.Add(H_SOLES, Decimal.Parse((CDbl((Decimal.Parse(dgw_mov1.Item(7, I).Value.ToString)))))), 2)
    ...
    Function MODIFICAR_TODO() As String
        ...
        End Try
        Try
            Dim comand1 As New SqlCommand("MODIFICAR_I_CON_CXP_CANCELACION", con)
            comand1.CommandType = CommandType.StoredProcedure
            comand1.Parameters.Add("@cod_aux", SqlDbType.VarChar).Value = COD_AUX
            comand1.Parameters.Add("@cod_comP", SqlDbType.VarChar).Value = COD_COMP
            comand1.Parameters.Add("@nro_comP", SqlDbType.VarChar).Value = txt_nro_comp.Text
