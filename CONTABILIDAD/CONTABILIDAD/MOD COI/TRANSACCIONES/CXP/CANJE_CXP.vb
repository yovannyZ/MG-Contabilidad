...
Public Class CANJE_CXP
    Public C As Boolean
    Dim COD_AUX, COD_COMP, BOTON, ESTADO_CTA, ORIGEN, DESTINO, COD_DH_DOC, COD_DH_GRAL, COD_MONEDA As String
    Dim obj As New Class1
    Private DT_DET As New DataTable
    Public E1 As Boolean
    Public M As Boolean
    Public N As Boolean
    Public P As Boolean
    ...
    Private Sub btn_doc_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_doc.Click
        ...
        Else
            ...
            Do While (l <> nro_cuotas)
                If obj.VERIFICAR_PCTAS_PAGAR("30", nro_doc01, TXT_COD.Text, txt_doc_per.Text, txt_origen.Text) = False Then
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_eliminar.Click
        ...
        End Try
        CARGAR_COMPROBANTE()
        If VERIFICAR_LETRA() AndAlso VERIFICAR_LETRA_CONFIRMADA() Then
            ...
            If (Decimal.Parse((CInt(MessageBox.Show(("Esta seguro de eliminar el Comprobante Nº " & NRO_PLAN), "Advertencia", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)))) = 6) Then
                ...
                End If
                Try
                    Dim cmd As New SqlCommand("ELIMINAR_I_CON_CXP_CANJE", con)
    ...
    Private Sub CANJE_CXP_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        If TabControl1.SelectedIndex <> 0 Then
            ValidarFicha = True
        End If
    End Sub

    Private Sub CANJE_CXP_Deactivate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Deactivate
        If TabControl1.SelectedIndex <> 0 Then
            ValidarFicha = False
        End If
    End Sub
    ...
    Sub CARGAR_COMPROBANTE()
        ...
        Try
            Dim PRO_02 As New SqlCommand("MOSTRAR_T_CON_CXP_CANJE", con)
            ...
            Do While RS3.Read
                ...
                Else
                    ...
                    Else
                        ...
                        'con.Open()
                        Dim cmd2 As New SqlCommand("OBTENER_AUXILIAR_COMP_EGRESO", con)
    ...
    Sub CARGAR_CUENTAS()
        Try
            ...
            pro.Parameters.Add("@TIPO_CUENTA", SqlDbType.Char).Value = "P"
    ...
    Sub CARGAR_PCTAS_PAGAR()
        dgw_det.Rows.Clear()
        Try
            Dim cmd As New SqlCommand("MOSTRAR_PCTAS_PAGAR_CANJE", con)
            cmd.CommandType = CommandType.StoredProcedure
            ...
            cmd.Parameters.Add("@CUENTA_ORIGEN", SqlDbType.VarChar).Value = txt_origen.Text
            cmd.Parameters.Add("@fecha_doc", SqlDbType.DateTime).Value = FECHA_GRAL.AddDays(1)
    ...
    Sub DATAGRID()
        Try
            Dim CMD As New SqlCommand("MOSTRAR_ICON_CXP_CANJE", con)
            CMD.CommandType = CommandType.StoredProcedure
    ...
    Sub HALLAR_TOTAL()
        ...
        Do While (I <= CONT)
            If dgw_det.Item(7, I).Value.ToString = "True" Then
                If dgw_det.Item(8, I).Value.ToString = "D" Then
    ...
    Private Sub txt_cta_origen_LostFocus(ByVal sender As Object, ByVal e As EventArgs) Handles txt_origen.LostFocus
        ...
        If (Strings.Trim(txt_origen.Text) <> "") Then
            If (dgw_cuenta.RowCount = 0) Then
                MessageBox.Show("No existen cuentas de Analisis P ", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Else
    ...
    Function VERIFICAR_GENERACION_LETRAS() As Boolean
        ...
        For I = 0 To CONT
            ...
            If obj.VERIFICAR_PCTAS_PAGAR("LT", NRO_DOC3, TXT_COD.Text, txt_doc_per.Text, txt_destino.Text) = False Then
    ...
    Public Function VERIFICAR_LETRA() As Boolean
        Dim I As Integer = 0
        Dim CONT As Integer = (DGW_DET2.Rows.Count - 1)
        I = 0
        Do While (I <= CONT)
            Dim NRO_DOC3 As String = DGW_DET2.Item(1, I).Value.ToString
            If obj.VERIFICAR_ELIMINAR_PCTAS_PAGAR("LT", NRO_DOC3, TXT_COD.Text, txt_doc_per.Text, txt_destino.Text) = False Then
    ...
    Public Function VERIFICAR_LETRA_CONFIRMADA() As Boolean
        Dim I As Integer = 0
        Dim CONT As Integer = (DGW_DET2.Rows.Count - 1)
        I = 0
        Do While (I <= CONT)
            Dim NRO_DOC3 As String = DGW_DET2.Item(1, I).Value.ToString
            If obj.VERIFICAR_ELIMINAR_PCTAS_PAGAR_CONFIRMADA("LT", NRO_DOC3, TXT_COD.Text, txt_doc_per.Text, txt_destino.Text) Then
                MessageBox.Show(("La Letra Nº " & NRO_DOC3 & " se encuentra confirmada."), "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Return False
            End If
            I += 1
        Loop
